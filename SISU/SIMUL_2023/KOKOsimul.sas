...
%LET AINEISTO = REK;	* Käytettävä aineisto (REK = Rekisteriaineisto);
...
%LET KEL_INDEKSIKOROTUS = 0; * 1 = Simuloiduille KEL-sidonnaisille etuuksille tehdään indeksikorotus;
...
/* Ajettavien osamallien valinta.
   ...
   2) ELASUMTUKI- ja OPINTUKI-malli pitää ajaa aina, jos ASUMTUKI-malli ajetaan.
...
%LET SAIRVAK = 1;
%LET TTURVA = 1; *1 = Vuosimalli, 2 = kuukausimalli;
%LET KOTIHTUKI = 1;
%LET KANSEL = 1;
%LET OPINTUKI = 1;
%LET VERO = 1;
...
%LET SIMUL_TIED_TT_KK = TTURVA_KK_simul;
...
%LET MUUTTUJAT = SAIRVAK_SIMUL SAIRVAK_DATA TTURVA_SIMUL TTURVA_DATA KOTIHTUKI_SIMUL KOTIHTUKI_DATA
		...
		EI_SIMULTULOT SEKAL_VAHENN OSVEROVAP_SIMUL OSVEROVAP_DATA 
...
%LET TARKISTUS_ASUMTUKI = 1;	* Tarkistetaan ASUMTUKI-mallin ajoa varten onko ELASUMTUKI ja OPINTUKI ajettuna (kyllä/ei (1/0));
...
%LET TULO = KAYTRAHATULO_SIMUL;  * Käytettävä tulokäsite: 
							   ...
							   - tai KAYTRAHATULO_KIRKVE_SIMUL (Käytettävissä olevat rahatulot, vähennetty kirkollisverot);
...
* Annetaan ERROR jos käyttäjä on valinnut ASUMTUKI-mallin ja lainsäädäntövuodelle 2017 tai 2025 simulointityypin SIMUL;
%IF &ASUMTUKI = 1 AND (&LVUOSI = 2017 OR &LVUOSI = 2025) AND %UPCASE(&TYYPPI_KOKO) = SIMUL %THEN %DO;
...
%IF &KOKOpoiminta = 1 %THEN %DO;
	...
	(KEEP = hnro knro asko tepalkat toptiot tosinktp apuraha tulkp telps43 tmuukust tepalk
	 tmerile tpalv trespa vthmp tpjta
	 tyhtat tepertyok1 tepertyok2 telps41 telps42 telps8 telps1 telps2 telps5 ttyoltuk tlakko tpalv2a
	 tomlis telvpal tsuurpu tutmp235 tutmp4
	 ...
	 tliik1evyr tliikpevyr tporo1evyr ymaatattuloevyr elyelattuloevyr tmaat2evyr
	 tmaat2pevyr tliik2evyr tliik2pevyr tporo2evyr ymaatpttuloevyr elyelpttuloevyr ttyosuhdematkal tpolkupyoraetuv tavainh
	);
	...
	PALKAT = SUM(tepalkat, toptiot, tosinktp, tulkp, telps43, tmuukust, tepalk, tmerile, tpalv, trespa, tutmp235, tutmp4);
	...
	MUU_ANSIO = SUM(tepertyok1, tepertyok2, telps41, telps42, telps8, telps1, telps2, telps5, ttyoltuk, tlakko, tpalv2a, tomlis, telvpal, tsuurpu, SUM(MAX(tkoultuk, 0)), apuraha, IFN(&LVUOSI >= 2025, vvvmk3, 0));

	/* Yritystuloja ansiotuloina (yrittäjävähennystä ei ole tehty) */

	YRIT_ANSIO = SUM(tmaat1evyr, tmaat1pevyr, tpjta, tliik1evyr, tliikpevyr, tporo1evyr, ymaatattuloevyr, elyelattuloevyr, SUM(tyhtat, -ymaatattulo, -elyelattulo), tmtatt);

	/* Yritystuloja pääomatuloina (yrittäjävähennystä ei ole tehty) */

	YRIT_POTULO = SUM(tmaat2evyr, tmaat2pevyr, tliik2evyr, tliik2pevyr, tporo2evyr, ymaatpttuloevyr, elyelpttuloevyr, SUM(ptyhtyma, -ymaatpttulo, -elyelpttulo), MAX(tmetsp, 0), MAX(tmetspp, 0), MAX(ymetspuptulo, 0), tvaksp);
	...
	OSINGOTP_DATA = SUM(tnoosvab, einotptosva, tuosvv, einotyjptva);
	OSINGOTP1_DATA = SUM(tnoosvab, tuosvv);
	OSINGOTP2_DATA = einotyjptva;
	OSINGOTP3_DATA = einotptosva;
	OSINGOTA_DATA = SUM(teinova, tpeito, einotpyjatva);
	YLEVERO_DATA = lylen;
	LAPSIP_DATA = SUM(MAX(bbyhte, 0), MAX(aitav, 0) , MAX(lbeltuki, 0));
	ELASUMTUKI_DATA = SUM(MAX(eastuki, 0));
	ASUMTUKI_DATA = SUM(MAX(yastuki, 0), 0);
	ASUMTUET_DATA = SUM(ASUMLISA_DATA, ELASUMTUKI_DATA, ASUMTUKI_DATA);
	PHOITO_DATA = SUM(hoimaksk, hoimakso);
	TOIMTUKI_DATA = SUM(MAX(totu_yhteensa, 0));
	...
	SAIRVAK_DATA TTURVA_DATA KANSEL_PERHEL_DATA VEROTT_KANSEL_DATA KOTIHTUKI_DATA OPINTUKI_DATA
...
%IF (&TTURVA = 1 OR &TTURVA = 2) AND &VERO = 0 %THEN %PUT WARNING: TTURVA-malli valittu, valitse myös VERO-malli!;
...
%IF &TTURVA = 2 %THEN %DO;
	%INCLUDE "&LEVY&KENO&HAKEM&KENO.SIMUL_&MVUOSI&KENO&SIMUL_TIED_TT_KK..sas";
%END;
...
%IF &SAIRVAK = 1 OR &TTURVA = 1 OR &TTURVA = 2 OR &KANSEL = 1 OR &KOTIHTUKI = 1 OR &OPINTUKI = 1 OR &VERO = 1 OR &KIVERO = 1
	...
	DATA STARTDAT.START_KOKO;
		...
		%IF &KANSEL = 1 %THEN %DO;
			TEMP.&TULOSNIMI_KE
			(KEEP = hnro TAKUUELA KANSANELAKE LAPSIKOROT RILISA YLIMRILI EHOITUKI LVTUKI VTUKI
		...
		%IF &VERO = 1 %THEN %DO;
			TEMP.&TULOSNIMI_VE
				(KEEP = hnro PALKVAK THANKKULUT2 PUHD_ANSIO PUHD_PO ANSIOT_VAH ELTULVAH ELTULVAH_K ELTULVAH_V OPRAHVAH
				INVVAH_K INVVAH_V PRAHAMAKSU KUNNVTULO1 KUNNVTULO2 PERVAH PERVAH_K LAPSIVAH KUNNVEROG KIRKVEROG SAIRVAKG KEVG
				VALTVERTULO YHTHYV VALTVEROH TYOTULVAH ELVELV_VAH POVEROC ALIJHYV ALIJHYVERIT
				KOTITVAH_DATA KOTITVAH ULKVAH OSINKOVAP OSINKOP OSINKOA OSINKOP1 OSINKOP2 OSINKOP3
				OSINKOVAP1 OSINKOVAP2 OSINKOVAP3 OSINKOVAP4 ASKOROT ENSASKOROT PO_VAHENN ANSIOVEROT
				KAIKKIVEROT MAKSP_VEROT YLEVERO VERTULO1 VERTULO2)
		%END;
...
%ELSE %DO;
	...
	VEROTT_KANSEL_SIMUL = SUM(LAPSIKOROT, RILISA, YLIMRILI, EHOITUKI, LVTUKI, VTUKI, MMTUKI);
%END;
...
%END;
%ELSE %DO;
	PHOITO_SIMUL = SUM(PHMAKSU_KOK, PHMAKSU_OS, 0);
%END;
...
KAYTTULO_DATA = KAYTRAHATULO_DATA;

KAYTTULO_SIMUL = KAYTRAHATULO_SIMUL;
...
ARRAY PISTE 
	...
	EI_SIMULTULOT SEKAL_VAHENN OSVEROVAP_DATA OSVEROVAP_SIMUL OSINGOTP_DATA 
