/*********************************************************
* Kuvaus: Eläkkeensaajan asumistuen simulointimalli 2018 *
...
%IF &START NE 1 %THEN %DO;
	...
	* Inflaatiokorjaus. Euro- tai markkamääräisten parametrien haun yhteydessä suoritettavassa
	...
	%LET MUUTTUJAT = aemkm ELAKASUMTUKI; 			/* Summataulukoissa taulukoitavat muuttujat */
...
%MACRO ElAsumTuki_Muut_Poiminta;
	...
	%IF &POIMINTA = 1 %THEN %DO;
		...
		DATA TEMP.TEMP_ELASUMTUKI_PERUS; 
			...
			halpinta maksvuok kaytkorv yhtiovas lisamaks lisalamm omamaks omalamm aslaikor tontvuok jasenia
			ikavu tkansel omalaji tperhel perlaji tansel tklaji velaji ttapel tlapel takuuel tulkelvp aslaji
			eastukikr yastukikr elak paos svatva svatvp teinova tpeito toyjmyv tnoosvab tenosve tuosvv toyjmav
			teinovv tnoosvvb teinovvb tuosvvap toyjmyvvap toyjmavvap kokorve opirake opirako evlamm rakvuosi yrvah);
		RUN;
		...
		PROC SQL;
			...
			AS SELECT hnro,
				...
				(SUM(kaytkorv)/jasenia)*12 AS KAYTKORVJ12,
				(SUM(yhtiovas)/jasenia)*12 AS YHTIOVASJ12,
				SUM(lisamaks)/jasenia AS LISAMAKSJ,
				SUM(lisalamm)/jasenia AS LISALAMMJ,
				SUM(omamaks)/jasenia AS OMAMAKSJ,
				SUM(omalamm)/jasenia AS OMALAMMJ,
				SUM(aslaikor)/jasenia AS ASLAIKORJ,
				SUM(tontvuok)/jasenia AS TONTVUOKJ
				...
				%IF &LVUOSI < 2015 %THEN %DO;
					...
					(
						...
						(
							...
							(((tkansel > 0) AND (omalaji NOT IN (1,2,10,90))) OR ((tperhel > 0) AND (perlaji IN (1,2,3))))
							OR
							/* saa kansaneläkelain mukaista vanhuuseläkettä eläketuen perusteella */
				...
				%ELSE %DO;
					...
					(
						(
							...
							((tkansel > 0) OR ((tperhel > 0) AND (perlaji IN (1,2,3))))
							OR
							/* saa takuueläkettä */
							(takuuel > 0)
							OR
						...
						(NOT ((velaji = 6) AND ((tperhel = 0) AND (perlaji IN (1,2,3)))))
					)
				%END;
		...
		PROC SQL;
			...
			AS SELECT a.hnro, a.knro, a.ELASUMTUKI_YKSIKKO, a.ONOIKEUSYKS, a.PONOIKEUSYKS, a.VONOIKEUSYKS,
				b.HALPINTAJ, b.MAKSVUOKJ12, b.KAYTKORVJ12, b.YHTIOVASJ12, b.LISAMAKSJ, b.LISALAMMJ, b.OMAMAKSJ, b.OMALAMMJ, b.ASLAIKORJ, b.TONTVUOKJ,
				c.puoliso, c.tperhel, c.perlaji, c.tkansel, c.omalaji, c.ikavu, c.aslaji, c.eastukikr, c.yastukikr, c.elak, c.paos,
				c.svatva, c.svatvp,	c.teinova, c.tpeito, c.toyjmyv, c.tnoosvab, c.tenosve, c.tuosvv, c.toyjmav,
				c.teinovv, c.tnoosvvb, c.teinovvb, c.tuosvvap, c.toyjmyvvap, c.toyjmavvap, c.kokorve, c.opirake, c.opirako,
				c.evlamm, c.rakvuosi, c.yrvah		
		...
		DATA STARTDAT.START_ELASUMTUKI_HENKI;
			...
			SAAYLLESK = ((ONOIKEUSYKS = 1) AND ((tperhel > 0) AND (perlaji IN (1,2,3))));	
			...
			PSAAVARVANH = ((PONOIKEUSYKS = 1) AND (((tkansel > 0) AND (omalaji = 2)) AND ((16 <= ikavu) AND (ikavu < 65))));  	
			...
			ELSE IF ONOIKEUSYKS = 1 AND SAAYLLESK = 0 THEN ELAKEKUUKAUDET = MAX(SUM(elak,-paos), 0);
			
			/* Saadut korot (e/v), lähdeveronalaiset; vanhempiensa kautta oikeutetuille 0 */
			KOROT = IFN(((ONOIKEUSYKS = 1) OR (PONOIKEUSYKS = 1)), MAX(kokorve, 0), 0);
			...
			VERONALOSINGOT_DATA = IFN(((ONOIKEUSYKS = 1) OR (PONOIKEUSYKS = 1)), SUM(teinova, tpeito, toyjmyv, tnoosvab, tenosve, tuosvv, toyjmav, 0), 0);
			...
			OPRAHA_DATA = IFN(((ONOIKEUSYKS = 1) OR (PONOIKEUSYKS = 1)), SUM(opirake, opirako, 0), 0);
			...
			LABEL
				...
				KAYTKORVJ12 = 'Vuokra-asunnon käyttökorvaukset (vesi, lämmitys ym.) kotitalouden jäsentä kohden (e/v), DATA'
				YHTIOVASJ12 = 'Osakeasunnon yhtiövastike kotitalouden jäsentä kohden (e/v), DATA'
				LISAMAKSJ = 'Osakeasunnon vesi, jäte ym. maksut hoitovastikkeen lisäksi kotitalouden jäsentä kohden (e/v), DATA'
				LISALAMMJ = 'Osakeasunnon lämmityskulut hoitovastikkeen lisäksi kotitalouden jäsentä kohden (e/v), DATA'
				...
				TONTVUOKJ = 'Omakotitalon tontin vuokrat kotitalouden jäsentä kohden (e/v), DATA'
				...
				KOROT = 'Saadut korot (e/v); vanhempiensa kautta oikeutetuille 0, DATA'
			...
			KEEP hnro knro ELASUMTUKI_YKSIKKO
				...
				HALPINTAJ MAKSVUOKJ12 KAYTKORVJ12 YHTIOVASJ12 LISAMAKSJ LISALAMMJ OMAMAKSJ OMALAMMJ ASLAIKORJ TONTVUOKJ
				ONPUOLISO SAAYLLESK PSAAVARVANH OMAKOTITALO KAYTKRYHMA ELAKEKUUKAUDET KOROT
				VERONAL_DATA VERONALOSINGOT_DATA VEROTTOSINGOT_DATA OPRAHA_DATA
				evlamm rakvuosi;
		...
		PROC SQL;
			...
			AS SELECT 
				...
				SUM(KAYTKORVJ12) AS KAYTKORVJ12YS,
				SUM(YHTIOVASJ12) AS YHTIOVASJ12YS,
				SUM(LISAMAKSJ) AS LISAMAKSJYS,
				SUM(LISALAMMJ) AS LISALAMMJYS,
				SUM(OMAMAKSJ) AS OMAMAKSJYS,
				SUM(OMALAMMJ) AS OMALAMMJYS,
				SUM(ASLAIKORJ) AS ASLAIKORJYS,
				SUM(TONTVUOKJ) AS TONTVUOKJYS,
				MAX(ELAKEKUUKAUDET) AS ELAKEKUUKAUDETY,
				SUM(KOROT) AS KOROTYS,
		...
		DATA STARTDAT.START_ELASUMTUKI_YKSIKKO;
			...
			ERILVESI = (SUM(KAYTKORVJ12YS, LISAMAKSJYS, OMAMAKSJYS) > 0);

			/* Yksikkö maksaa asunnosta erillisiä lämmitysmaksuja (0/1) */
			ERILLAMPO = (SUM(LISALAMMJYS, OMALAMMJYS) > 0);

			/* Yksikön maksama vuokra tai yhtiövastike ja mahdollinen tontinvuokra yhteensä (e/v) */
			VUVATO = SUM(MAKSVUOKJ12YS, YHTIOVASJ12YS, TONTVUOKJYS, 0);
			...
			LABEL 
				...
				KOROTYS = 'Yksikön saamat korot (e/v), DATA'
				...
				VUVATO = 'Yksikön maksama vuokra tai yhtiövastike ja mahdollinen tontinvuokra yhteensä (e/v), DATA';

			KEEP knro ELASUMTUKI_YKSIKKO
				SAAYLLESKY ONPUOLISOY VONOIKEUSYKSYS
				OMAKOTITALOY KAYTKRYHMAY HALPINTAJYS ASLAIKORJYS
				ELAKEKUUKAUDETY
				KOROTYS VERONALYS_DATA VERONALOSINGOTYS_DATA VEROTTOSINGOTYS_DATA OPRAHAYS_DATA
				EVLAMMY RAKVUOSIY
...
%MACRO ElAsumTuki_Simuloi_Data;
	...
	DATA TEMP.TEMP_ELASUMTUKI_YKSIKKO_TULOS;
		...
		IF SUM(KOROTYS, VERONALOSINGOTYS, VEROTTOSINGOTYS) > 60 THEN HUOMTULOT = SUM(VERONALYS, KOROTYS, VEROTTOSINGOTYS, -OPRAHAYS);
		...
		%ElakAsumTukiVS(ELAKASUMTUKI, &LVUOSI, &INF, ONPUOLISOY, MOLEOIKV, SAAYLLESKY, 0, VONOIKEUSYKSYS,
			...
			HUOMTULOT, 0, VUVATO, ASLAIKORJYS);
	...
	%IF &START NE 1 %THEN %DO;
		...
		DATA TEMP.&TULOSNIMI_EA;
			...
			%IF &TULOSLAAJ = 1 %THEN %DO; 
				...
				(KEEP = hnro knro &PAINO aemkm ikavu ikavuv desmod soss paasoss elivtu koulas koulasv rake maakunta)
				TEMP.&TULOSNIMI_EA;
			%END;
			...
			/* Annetaan simuloiduille muuttujille selitteet */
			LABEL 
			aemkm = 'Eläkkeensaajan asumistuki, DATA';
