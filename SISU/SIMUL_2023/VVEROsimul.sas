/****************************************************
* VVERO-mallin simulointiohjelma         			*
****************************************************/
...
%MACRO Aloitus;
	...
	%LET MUUTTUJAT = ALV KULUTUS /* ALV ja kulutus summatasolla */
			...
			_01_KULUTUS _01_ALV /* Kulutus ja ALV tuoteluokittain, esimerkkinä elintarvikkeet (tuoteluokka _01) */
			;
	...
	%LET PAINO = koraika; /* Käytettävä painokerroin.*/
...
%MACRO Alv_Muutt_Poiminta;
	...
	DATA STARTDAT.STARTDAT_ALV;
		SET POHJADAT.&AINEISTO&AVUOSI (KEEP = jlkm &PAINO &KULUYKS _01_13 kjnro &HAETTAVA_BRUTTO oecdmod
	...
	PROC SORT DATA = STARTDAT.STARTDAT_ALV; BY kjnro %VarExist(asko, STARTDAT.STARTDAT_ALV); RUN;
	...
	%Desiilit(kjnro, &TULO, jlkm, &KULUYKS, &PAINO, STARTDAT.STARTDAT_ALV, luokittelu = DESMOD);
...
%MACRO Alv_Simuloi_Data;
	...
	%Desiilit(kjnro, &TULO._ALV, jlkm, &KULUYKS, &PAINO, OUTPUT.&TULOSNIMI_VVE);
...
%MACRO KutsuTulokset;
	...
	%KoyhInd(3, 40, 50, 60, OUTPUT.&TULOSNIMI_VVE, jlkm, &PAINO, &TULO._ALV, &KULUYKS, kjnro, DESMOD_MALLI, 1);
