...
%IF &POIMINTA = 1 %THEN %DO;
	...
	%IF ((&LVUOSI >= 2021) AND (&LVUOSI <= 2023)) %THEN %DO; 
		%LET LAYRI = %SUBSTR(&LVUOSI, 3, 2);
		RENAME ayri&LAYRI = AYRI kayri&LAYRI = KAYRI;
	%END;

	%ELSE %IF &LVUOSI < 2021 %THEN %DO;
		RENAME ayri21 = AYRI kayri21 = KAYRI; 
	%END;
	...
	****************************************************************************************************************************************************;
	...
	%IF &LVUOSI>=2023 %THEN %DO; 
		...
		**********************************************;
		...
		/* Valtion tulovero */
		/* Poikkeukset sotemuutoksiin Ahvenanmaalle */
		IF VERTULO > 0 THEN DO;
				IF maakunta=21 THEN DO;
					%ValtTuloVeroS_sote(VALTVEROA, &LVUOSI, &INF, VERTULO);
				END;
				ELSE DO;
