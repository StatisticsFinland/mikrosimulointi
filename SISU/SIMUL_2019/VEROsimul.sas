...
*  Viimeksi päivitetty: 27.1.2023 TO  		 				 * 
* ***********************************************************/
...
%IF &START NE 1 %THEN %DO;
	...
	* Inflaatiokorjaus. Euro- tai markkamääräisten parametrien haun yhteydessä suoritettavassa
	...
	%LET SOTE = 0;* 1=Sote-muutokset käyttöön, 0=ei sotemuutoksia (vanhat lait);
...
%IF &POIMINTA = 1 %THEN %DO;
	...
	%IF ((&LVUOSI >= 2019) AND (&LVUOSI <= 2021)) %THEN %DO; 
		%LET LAYRI = %SUBSTR(&LVUOSI, 3, 2);
		RENAME ayri&LAYRI = AYRI kayri&LAYRI = KAYRI;
	%END;

	%ELSE %IF &LVUOSI < 2019 %THEN %DO;
		RENAME ayri19 = AYRI kayri19 = KAYRI; 
	%END;
...
%IF (&LVUOSI > 2021) and &AVUOSI ne 2019 %THEN %DO;
