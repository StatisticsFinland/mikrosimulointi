...
/* Makro osinkojen sarjatason laskentaan.
	...
	suorlaji: 			Suorituslaji
						1 Osinko
						2 Osuuspääoman korko (ennen vuotta 2015)
						3 Reit-osingot asuntojen vuokraustoiminnasta
						4 VOPR-osinko
						5 Osuuskunnan ylijäämä (vuodesta 2015)
						6 Osuuskunnan VOPR-ylijäämä (vuodesta 2015)
...
%macro OsinkojenJakoErillis(
		...
		---------------------------------------------------------------------*/
		if &jaktyypp = "M" and &suorlaji in (1, 4) then do;
		...
		---------------------------------------------------------------------*/
		else if (&jaktyypp = "J" and &suorlaji in (1, 4)) 
			or (&jaktyypp = '' and &suorlaji in (1, 4))
			then do;
				LIST_BR = &osinkoe;
		...
		Noteeraamattomat REIT-yritykset:
			- kokonaan veronalaista, mutta jaetaan kuin muutkin
			  noteeraamattomat yritykset
		---------------------------------------------------------------------*/
		else if &jaktyypp = "M" and &suorlaji = 3 then do;
		...
		else if &jaktyypp = "J" and &suorlaji = 3 then do;
		...
		else if (&jaktyypp = "M" and &suorlaji in (2, 5, 6))
			or (&jaktyypp = '' and &suorlaji in (2, 5, 6))
			then do;
		...
		-----------------------------------------------------------------------------*/
		else if (&jaktyypp = "S" and &suorlaji in (2, 5, 6)) then do;
		...
		else if (&jaktyypp = "J" and &suorlaji in (2, 5, 6))
			then do;
				LISTOS_BR = &osinkoe;
