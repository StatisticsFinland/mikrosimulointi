/*******************************************************************
*  Kuvaus: Sairausvakuutuksen päivärahojen simuloimalli 2018       * 
...
%IF &START NE 1 %THEN %DO;
	...
	%LET AINEISTO = REK_mk;	* Käytettävä aineisto (PALV = Palveluaineisto, REK = Rekisteriaineisto) ;
	...
	%LET PERHEVAP = 0; *Käytetäänkö vuoden 2022 perhevapaauudistuksen korotettujen vanhempainpäivärahapäivien määriä (=1 käytetään, =0 ei käytetä) ;
...
* Annetaan ERROR jos käyttäjä on valinnut PERHEVAP=1 lainsäädäntövuodelle ja kuukaudelle ennen 08/2022;
%IF &PERHEVAP = 1 AND (&LVUOSI < 2022 OR (&LVUOSI=2022 AND &LKUUK<8)) %THEN %DO;
	%PUT ERROR: Arvoa PERHEVAP=1 voi käyttää vain lainsäädöntövuoden 2022 elokuusta alkaen ;
	%ABORT CANCEL;
%END;
...
%IF &POIMINTA = 1 %THEN %DO;
	...
	muuperu aivpvkt aivpvt2 aivpvk aivpv2 aivpv1 aivpvt1 aivpvkt_u aivpvt2_u aivpvk_u aivpv2_u aivpv1_u aivpvt1_u tosinktp
	...
	WHERE ppv > 0 OR ppvt > 0 OR tulosrt > 0 OR tuloprt > 0 OR haiprva > 0
...
ELSE DO;
	...
	IF &LVUOSI >= 2021 AND 2019 <= &AVUOSI <= 2021 THEN DO;
		/* Vakuutetulle maksetut */
		%VanhPRahaVS(AIT_VANH, &LVUOSI, &INF, 1, 0, 0, 0, SUM(tuloprt, tosinktp) /(1 - &PalkVah));
		%VanhPRahaVS(KOR_VANH, &LVUOSI, &INF, 0, 1, 0, 0, SUM(tuloprt, tosinktp) / (1 - &PalkVah));
		%VanhPRahaVS(NORMVANH, &LVUOSI, &INF, 0, 0, 1, 0, SUM(tuloprt, tosinktp) / (1 - &PalkVah));
		/* Työnantajalle maksetut */
		%VanhPRahaVS(AIT_VANH_TYONANT, &LVUOSI, &INF, 1, 0, 0, 0, SUM(tuloprt, tosinktp) / (1 - &PalkVah));
		%VanhPRahaVS(KOR_VANH_TYONANT, &LVUOSI, &INF, 0, 1, 0, 0, SUM(tuloprt, tosinktp) / (1 - &PalkVah));
		%VanhPRahaVS(NORMVANH_TYONANT, &LVUOSI, &INF, 0, 0, 1, 0, SUM(tuloprt, tosinktp) / (1 - &PalkVah));
	END;
	/* Muulloin */
	ELSE DO;
		/* Vakuutetulle maksetut */
		%VanhPRahaVS(AIT_VANH, &LVUOSI, &INF, 1, 0, 0, 0, tuloprt /(1 - &PalkVah));
		%VanhPRahaVS(KOR_VANH, &LVUOSI, &INF, 0, 1, 0, 0, tuloprt / (1 - &PalkVah));
		%VanhPRahaVS(NORMVANH, &LVUOSI, &INF, 0, 0, 1, 0, tuloprt / (1 - &PalkVah));
		/* Työnantajalle maksetut */
		%VanhPRahaVS(AIT_VANH_TYONANT, &LVUOSI, &INF, 1, 0, 0, 0, tuloprt / (1 - &PalkVah));
		%VanhPRahaVS(KOR_VANH_TYONANT, &LVUOSI, &INF, 0, 1, 0, 0, tuloprt / (1 - &PalkVah));
...
/* Kerrotaan päivillä */
/* Vuoden 2022 perhevapaauudistuksessa käytetään uusia päivämuuttujia */

IF  &PERHEVAP= 1 THEN DO;
	AIT_VANH = aivpvk_u * AIT_VANH;
	KOR_VANH = aivpv1_u * KOR_VANH;
	NORMVANH = aivpv2_u * NORMVANH;
	AIT_VANH_TYONANT = aivpvkt_u * AIT_VANH_TYONANT;
	KOR_VANH_TYONANT = aivpvt1_u * KOR_VANH_TYONANT;
	NORMVANH_TYONANT =  aivpvt2_u * NORMVANH_TYONANT;
END;
...
IF muuperu = "TT" THEN DO;
	%AnsioSidVS(ANSIOSID, &LVUOSI, &INF, 0, 0, 0, 0, ANSPALKKA, 0);
...
%IF &START NE 1 %THEN %DO;
	...
	%IF &TULOSLAAJ = 1 %THEN %DO; 
		...
		(KEEP = hnro knro &PAINO hsaiprva haiprva pmkyt amkyt hwmky ikavu ikavuv desmod soss paasoss elivtu koulas koulasv rake maakunta)
		TEMP.&TULOSNIMI_SV;
	%END;
	...
	* Asetetaan muuttujien 0-arvot tyhjiksi, jotta lukumäärät voidaan laskea suoraan ;
	...
	ARRAY PISTE 
		hsaiprva haiprva pmkyt amkyt hwmky;
	DO OVER PISTE;
		IF PISTE <= 0 THEN PISTE = .;
	END;
