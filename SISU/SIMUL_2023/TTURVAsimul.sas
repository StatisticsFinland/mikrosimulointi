...
%IF &POIMINTA = 1 %THEN %DO;
	...
	vvvmk5 mkorpvt5
	dtyhtet dtpalkmt dtyhtep dtpalkmp ttyotpr dtvpv dttayspv dtospv dtovlkm dtopalkk vanh_omaishoitohp
	dttspv dtomapal dtpspv dtpspalk dtyllae dtyllaep dtyllapv dttllkm dtthpv dttspalk 
	dtyllapvp koropvtkg
	koropvpkw spalkka3 ikavu tklaskr:
	...
	TMTUKIDAT = SUM(dtyhtet, -dtyllae);
	PERPRDAT = SUM(dtyhtep, -dtyllaep);
	...
	*Jos työmarkkinatuen taustatiedot ovat tyhjät, täydennetään niitä
	...
	ARRAY PISTE 
	dtyhtet dtyllae dtyhtep dtyllaep vvvmk3 VVVMKQ VVVPVTQ;
	DO OVER PISTE;
	...
	dtpalkmt = 'Työmarkkinatuen päivien lkm, DATA'
	...
	dtopalkk = 'Vanhempien työtulot ositetussa tm-tuessa, DATA'
	vanh_omaishoitohp = 'Vanhempien omaishoidon tuen hoitopalkkiot ositetussa tm-tuessa, DATA'
	TMTUKIDAT = 'Työmarkkinatuki ilman kulukorvauksia, DATA'
	PERPRDAT = 'Peruspäiväraha ilman kulukorvauksia, DATA'
...
/* 3.1.1. Päivien uudelleenjako ansiosidonnaisen päivärahan enimmäiskeston perusteella */
	...
	*Muodostetaan täyden tuen määrä, jota käytetään apuna monessa kohtaa simulointia;
	%AnsioSidVS(TAYSANSPR, &LVUOSI, &INF, VVVLLKMQ, 0, 0, 0, PALKKA, 0, MAX(tklaskr1,tklaskr2,tklaskr3,tklaskr4), max(0, tklaskr4_ed));
	...
	*Keston simulointi alkaa;
	IF &APKESTOSIMUL=1 THEN DO;
		...
		*Simuloidaan tm-tuen kuukausimäärät nettopäivien määrittämiseen;
		IF dttspv > 0 THEN DO;
			%TyomTukiVS(TEMP_TAYSTM, &LVUOSI, &INF, 0, 0, 0, dttllkm, 0, 0, 0, 0, 0, 0, 0);
		...
		END;
		TMPAIVAT_NETTO = SUM(dtpalkmt, -dttspv, ROUND(SOVSUHDETM_LVUOSI * dttspv));
		...
		*Jyvitetään siirtyvät ansiopäivärahat työmarkkinatukeen lajeittain vastaten samoja suhteita kuin ansiopäivärahassa; 
		...
		IF SIIRTPV < 0 THEN DO;
			...
			*Vähennys ansiopäivärahoista;
			...
			IF SOVSUHDETM_LVUOSI > 0 AND ROUND(dttspv/dtpalkmt * (-SIIRTPV)) > 0 THEN DO;

				*Painotettu keskiarvo työtuloista;
				SOVPALKKAQ = SUM(ROUND(dttspv/dtpalkmt * (-SIIRTPV)) * SOVPALKKATM, VVVSOPVQ * SOVPALKKAQ) / SUM(ROUND(dttspv/dtpalkmt * (-SIIRTPV)), VVVSOPVQ);
...
/* 3.1.2. Ansiopäivärahan simulointi */
	...
	* Lasketaan korotetut päivärahat ;
	IF SOVKORPV1 OR TAYSKORPV1 THEN DO;
		%AnsioSidVS(TAYSKORPR1, &LVUOSI, &INF, VVVLLKMQ, (&LVUOSI < 2017 AND KOKOTYOHISTV > &KorEhtoV), 0, 0, PALKKA, 0, MAX(tklaskr1,tklaskr2,tklaskr3,tklaskr4), max(0, tklaskr4_ed));
	...
	* Lasketaan korotetut päivärahat ;
	IF SOVKORPV5 OR TAYSKORPV5 > 0 THEN DO;
		%AnsioSidVS(TAYSKORPR5, &LVUOSI, &INF, VVVLLKMQ, 1, 0, 0, PALKKA, 0, MAX(tklaskr1,tklaskr2,tklaskr3,tklaskr4), max(0, tklaskr4_ed));
...
/* 3.1.3. Työmarkkinatuen simulointi (voimassa 4/2026 asti) */
...
IF TMTUKIPV_SIMUL > 0 THEN DO;
	...
	* Tarveharkitut työmarkkinatuet ;
	IF TAYSTARV OR SOVTARV THEN DO;
		...
		IF &TTDATATULO NE 0 THEN DO;
			%TyomTukiVS(THTMTUKI, &LVUOSI, &INF, 1, 0, PUOLPALK, dttllkm, 0, dtomapal, PUOLPALK, 0, 0, 0, 0);
			IF SOVTARV > 0 THEN DO; %SoviteltuVS(SOVTARVTUKI, &LVUOSI, &INF, 0, 0, dttllkm, THTMTUKI, dttspalk, 0, 0); END;
		END;
	
		ELSE DO;
			%TyomTukiVS(THTMTUKI, &LVUOSI, &INF, 1, 0, PUOLPALK, dttllkm, 0, DTOMAPALX2, PUOLPALK, 0, 0, 0, 0);
	...
	* Osittaiset työmarkkinatuet ;
	IF TAYSOS OR SOVOS THEN DO;
		...
		IF &TTDATATULO NE 0 THEN DO;
			%TyomTukiVS(OSTMTUKI, &LVUOSI, &INF, 0, 1, 0, dttllkm, dtovlkm, 0, 0, dtopalkk, vanh_omaishoitohp, 0, 0);
			IF SOVOS > 0 THEN DO; %SoviteltuVS(SOVOSTUKI, &LVUOSI, &INF, 0, 0, dttllkm, OSTMTUKI, dttspalk, 0, 0); END;
		END;
	
		ELSE DO;
			%TyomTukiVS(OSTMTUKI, &LVUOSI, &INF, 0, 1, 0, dttllkm, dtovlkm, 0, 0, DTOPALKKX2, vanh_omaishoitohp, 0, 0);
	...
	* Korotetut työmarkkinatuet;
	IF PELKKOR OR KORVAH OR SOVKOR OR SOVKORVAH THEN DO;
		IF PELKKOR > 0 OR SOVKOR > 0 THEN DO;
			%TyomTukiVS(KRTMTUKI, &LVUOSI, &INF, 0, 0, 0, dttllkm, 0, 0, 0, 0, 0, 1, 0);
		END;
		IF KORVAH > 0 OR SOVKORVAH > 0 THEN DO;
			%TyomTukiVS(KRSTMTUKI, &LVUOSI, &INF, 0, 0, 0, dttllkm, 0, 0, 0, 0, 0, 1, SOSETUVAH);
	...
	* Sovitellut täydet tmtuet ;
	IF SOVPELK > 0 THEN DO;
		...
		IF &TTDATATULO NE 0 THEN DO;
			%TyomTukiVS(TAYSPRAHA, &LVUOSI, &INF, (ikavu < &TarvHarkIka.) OR NOT MAX(dtyhtep, VVVMKQ), 0, PUOLPALK, dttllkm, 0, 0, PUOLPALK, 0, 0, 0, 0);
	...
	* Täydet tmtuet ;
	IF dttayspv > 0 THEN DO;
		%TyomTukiVS(TAYSTMTUKI, &LVUOSI, &INF, (ikavu < &TarvHarkIka.) OR NOT MAX(dtyhtep, VVVMKQ), 0, PUOLPALK, dttllkm, 0, 0, PUOLPALK, 0, 0, 0, 0);
		TAYSTMTUKI = dttayspv * TAYSTMTUKI / &TTPaivia;
	END;
	
	* Tuet joista on vähennetty muuta sosiaalietuutta ;
	IF PELKVAH > 0 OR SOVVAH > 0 THEN DO; 
		%TyomTukiVS(VAHTMTUKI, &LVUOSI, &INF, 0, 0, 0, dttllkm, 0, 0, 0, 0, 0, 0, SOSETUVAH); 
...
/* 3.1.4. Peruspäiväraha (voimassa 4/2026 asti) */
...
/* 3.1.5. Yleistuki (voimassa alkaen 5/2026, simuloidaan työmarkkinatuki- ja peruspäivärahapäivien perusteella) */
IF TMTUKIPV_SIMUL OR dtpalkmp THEN DO;

	* Yleistuki: simuloitu täysille työmarkkinatuki- ja peruspäivärahapäiville;
	...
	* Yleistuki: simuloitu tarveharkintaisen työmarkkinatukipäiville;
	IF TAYSTARV OR SOVTARV THEN DO;
		IF &TTDATATULO NE 0 THEN DO;
	...
	* Yleistuki: simuloitu osittaisena maksetuille työmarkkinatukipäiville;
	IF TAYSOS OR SOVOS THEN DO;
		IF &TTDATATULO NE 0 THEN DO;
	...
	* Yleistuki: simuloitu työmarkkinatukipäiville, joista on vähennetty muut sosiaalietuudet;
	...
	* Yleistuki: simuloitu sovitelluille työmarkkinatukipäiville;
	...
	* Yleistuki: simuloitu sovitelluille peruspäivärahapäiville;
...
%IF &START NE 1 %THEN %DO;
	...
	%IF &TULOSLAAJ = 1 %THEN %DO; 
		...
		(KEEP = hnro knro &PAINO dtyhtet YPITOKDAT dtyhtep AILMKORQDAT VVVMKQ vvvmk3 VVVPVTQ
	...
	* Asetetaan muuttujien 0-arvot tyhjiksi, jotta lukumäärät voidaan laskea suoraan ;
	...
	ARRAY PISTE 
	dtyhtet YPITOKDAT dtyhtep AILMKORQDAT VVVMKQ vvvmk3 VVVPVTQ ;
	DO OVER PISTE;
		IF PISTE <= 0 THEN PISTE = .;
	END;
	...
	* Luodaan datan muuttujille selitteet ;
	...
	LABEL 
	dtyhtet = 'Työmarkkinatuki, DATA'
	dtyhtep = 'Peruspäiväraha, DATA'
