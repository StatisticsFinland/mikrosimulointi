/************************************************************
*  Kuvaus: Tuloverotuksen simulointimalli		         	*
* ***********************************************************/
...
%IF &START NE 1 %THEN %DO;
	...
	* Inflaatiokorjaus. Euro- tai markkamääräisten parametrien haun yhteydessä suoritettavassa
	...
	%LET MUUTTUJAT = ANSIOT POTULOT KOKONTULO ltva VALTVEROH ltvp POVEROC lkuve KUNNVEROG lkive KIRKVEROG lshma SAIRVAKG lelvak PALKVAK 
					 lpvma PRAHAMAKSU KOTITVAH_DATA KOTITVAH lylen YLEVERO KAIKKIVEROT_DATA KAIKKIVEROT verot MAKSP_VEROT
					 OSINKOA_DATA OSINKOA OSINKOP_DATA OSINKOP OSINKOVAP_DATA OSINKOVAP; /* Taulukoitavat muuttujat, sotemuutokset(summataulukot) */
...
%IF &POIMINTA = 1 %THEN %DO;
	...
	SET POHJADAT.&AINEISTO&AVUOSI (KEEP = 
	 ...
	 invah82 invpros cllkm siviilisaaty fluotap fluotpt fkunvtapat ftapep ftapepp
	 ftappm ftappmp ftvuora ikakk ikavu
	 evlaplkm lelvak lkive lkuve lpvma lshma ltva ltvp lveru svatva svatvap
	 svatpp tansel apuraha teanstu teinova teinovv teinovvb teleuve telps1
	 telps2 telps5 telvpal einotptosva tepalk tjmark tjvkork tkansel telps43 tmuukust
	 tlakko tmerile tmetsp tmetspp tepalkat toptiot tosinktp tmtatt tmuuel tmuukor
	 tmuutp tmyynt tmyynt1 tnoosvab tnoosvvb tomlis einotyjptva toyjmavvap
	 einotpyjatva toyjmyvvap treitosva tpalv tepertyok1 tepertyok2 telps41 telps42 telps8 tpalv2a
	 ...
	 tyonosuu_remontti tyonosuu_koti tyonosuu_oljy palksiku_yht palkomos_yht tyonosuu_yht 
	 ymetspuptulo tulkelvp ymaatattulo elyelattulo ymaatpttulo elyelpttulo
	 tmaat1evyr tmaat1pevyr tliik1evyr tliikpevyr tporo1evyr ymaatattuloevyr
     elyelattuloevyr tmaat2evyr tmaat2pevyr tliik2evyr tliik2pevyr tporo2evyr
     ymaatpttuloevyr elyelpttuloevyr VKERROIN tulkelvp
	 );
	...
	%ELSE %IF &LVUOSI > 2026 %THEN %DO;
		RENAME ayri26 = AYRI kayri26 = KAYRI;
	%END;
	...
	SET STARTDAT.START_VERO (KEEP = knro hnro siviilisaaty asko);
	...
	MUU_TTURVA_DATA = SUM(MAX(tkoultuk, 0), IFN(&LVUOSI >= 2025, vvvmk3, 0)); 
	OPTUKI_DATA = tkopira;
	...
	OSINKOA_DATA = SUM(teinova, tpeito, einotpyjatva);
	OSINKOP_DATA = SUM(tnoosvab, einotptosva, tuosvv, einotyjptva);

	/* Puolison päättelyä */
	PSOT = IFN(siviilisaaty = 2, 1, 0);
	...
	ULKPALKKA = tulkp;
	PALKKA1 = SUM(tepalkat, toptiot, tosinktp, ULKPALKKA, telps43, tmuukust, tepalk, tmerile, tpalv, trespa, tutmp235, tutmp4);
	MUU_TYO = SUM(tepertyok1, tepertyok2, telps41, telps42, telps8, telps1, telps2, telps5, ttyoltuk);
	YRITYSTA = SUM(tmaat1evyr, tmaat1pevyr, tpjta, tliik1evyr, tliikpevyr, tporo1evyr, ymaatattuloevyr, elyelattuloevyr, SUM(tyhtat, -ymaatattulo, -elyelattulo), tmtatt);
	TYOTULOA = SUM(PALKKA1, MUU_TYO, YRITYSTA);
	YRITYSTP = SUM(tmaat2evyr, tmaat2pevyr, tliik2evyr, tliik2pevyr, tporo2evyr, ymaatpttuloevyr, elyelpttuloevyr, SUM(ptyhtyma, -ymaatpttulo, -elyelpttulo), MAX(tmetsp, 0), MAX(tmetspp, 0), MAX(ymetspuptulo, 0), tvaksp);
	VAKPALK = SUM(tepalkat,tosinktp, tmerile);
	MUUT_EL = MAX(SUM(tansel, ttapel, tlapel, tpotel, teanstu, tmuuel, teleuve, tulkelvp), 0); 	
	MUU_ANSIO = SUM(tlakko, tpalv2a, tomlis, telvpal, tsuurpu, MUU_TTURVA_DATA, apuraha);
	THANKK = SUM(vthm4);
	MUU_VAH_VALT2 = SUM(vmuut1, vmuutav);
	MUU_VAH_KUNN2 = SUM(vmuut1, vmuutk, fkunvtapat);
	MUU_VAH_VER = SUM(vmuut1, vmuutk, vmuutk, fkunvtapat);
	POTAPP = SUM(ftappm, ftapep, ftappmp, ftapepp, ftvuora);
	/* Muodostetaan muuttuja ULK_OSUUS niille henkilöille, joille ulkomaan tuloihin on käytetty vapautusmenetelmää */
	IF svatva > 0 AND htpuhdatvap > 0 THEN ULK_OSUUS = htpuhdatvap / svatva; ELSE ULK_OSUUS = 0;
	...
	MUU_PO = SUM(tjvkork, tmuukor, tjmark, tmuutp, tsiraho, 
				MAX(SUM(tmyynt, tmyynt1, -fluotap), 0), tvahevas, tptmuu, MAX(SUM(tulkyhp, -tuosvv),0),
					tvahep50, tptvs, tvahep20, tptsu50, tkapite);
	...
	IF 2009 <= &LVUOSI <= 2012 OR &LVUOSI >= 2016 THEN DO;
		...
		MUU_VAH_VER = SUM(MUU_VAH_VER, vlahj);
	END; 
	...
	KOTITVAH_DATA = SUM(vkotita, vkotitku, vkotitsh, vkotitki, vkotitp);
	...
	MUU_VAH_VER = 'Muita vähennyksiä verotuksessa (sotemuutos), DATA'
	...
	DROP siviilisaaty fluotap fluotpt fkunvtapat ftapep ftapepp ftappm ftappmp
		ftvuora lkive lkuve lpvma lshma ltva ltvp svatva svatvap svatpp
		tansel apuraha teanstu teleuve telps1 telps2 telps5 telvpal tepalk tjmark
		tjvkork tkansel telps43 tmuukust tlakko tepalkat toptiot tosinktp tmtatt tmuuel tmuukor
		tmuutp tmyynt tmyynt1 tomlis tpalv tepertyok1 tepertyok2 telps41 telps42 telps8 tpalv2a
		tperhel tpjta tpotel trespa tsiraho
		tsuurpu ttapel tlapel ttyoltuk htpuhdatvap tutmp235
		tutmp4 tvahevas tptmuu tvaksp tvuokr tvuokr1
		vkotita vkotitki vkotitku vkotitp vkotitsh vlahj vmuut1 vmuutk
		vmuutav vthm4 vvvmk1 vvvmk3 vvvmk5 saiprva aiprva cdmky
		tkoultuk dtyhtet tkopira takuuel tvahep50 tptvs tvahep20 tptsu50
		dtyhtep
...
%IF &SAIRVAK = 1 OR &TTURVA = 1 OR &TTURVA = 2 OR &KANSEL = 1 OR &KOTIHTUKI = 1 OR &OPINTUKI = 1 %THEN %DO;
	...
	DATA STARTDAT.START_VERO;
		...
		%IF &TTURVA = 1 %THEN %DO;
			TEMP.&TULOSNIMI_TT
			(KEEP = hnro YHTTMTUKI TMTUKILMKOR PERILMAKOR PERUSPR ANSIOPR ANSIOILMKOR VUORKORV YLEISTUKI)
		%END;

		%IF &TTURVA = 2 %THEN %DO;
			TEMP.&TULOSNIMI_TT
			(KEEP = hnro YHTTMTUKI TMTUKILMKOR PERILMAKOR PERUSPR ANSIOPR ANSIOILMKOR YLEISTUKI)
		%END;
...
/* 4. Nämä määrittelyt ja makro tuottavat kertoimet keskimääräisen kunnallis- ja kirkollisveroprosentin muunnoskertoimen laskemiseksi */
...
%MACRO Vero_Simuloi_Data;
	...
	/* Luodaan tyhjät lokaalit muuttujat lakiparametrien hakua varten */
	...
	%ELSE %IF &TTURVA = 2 %THEN %DO;
		TTURVA_SIMUL = SUM(YHTTMTUKI, PERUSPR, ANSIOPR, YLEISTUKI);
	%END;
	%ELSE %DO; 
		TTURVA_SIMUL = SUM(YHTTMTUKI, PERUSPR, ANSIOPR, VUORKORV, YLEISTUKI);
	%END;
	...
	%ELSE %DO;
		...
		%OsinkojenJakoS(OSINKOA, &LVUOSI, 1, 0, 3, 0, SUM(teinova, teinovv, einotpyjatva, toyjmyvvap), 0) ;
		...
		%OsinkojenJakoS(OSINKOP2, &LVUOSI, 1, 1, 2, 0, SUM(einotyjptva, toyjmavvap), 0);
		%OsinkojenJakoS(OSINKOP3, &LVUOSI, 1, 0, 2, 0, SUM(einotptosva, teinovvb), -1);
		...
		%OsinkojenJakoS(OSINKOVAP2, &LVUOSI, 1, 1, 1, 0, SUM(einotyjptva, toyjmavvap), 0);
		%OsinkojenJakoS(OSINKOVAP3, &LVUOSI, 1, 0 ,1, 0, SUM(einotptosva, teinovvb), -1);
		%OsinkojenJakoS(OSINKOVAP4, &LVUOSI, 1, 0, 1, 0, SUM(teinova, teinovv, einotpyjatva, toyjmyvvap), 0);
	...
	/* Yrittäjävähennys */
	%YrittajaVahS(YRVAHA, &LVUOSI, &INF, SUM(tmaat1evyr, tmaat1pevyr, tliik1evyr, tliikpevyr, tporo1evyr, ymaatattuloevyr, elyelattuloevyr));
	%YrittajaVahS(YRVAHP, &LVUOSI, &INF, SUM(tmaat2evyr, tmaat2pevyr, tliik2evyr, tliik2pevyr, tporo2evyr, MAX(tmetsp, 0), MAX(tmetspp, 0), MAX(ymetspuptulo, 0), ymaatpttuloevyr, elyelpttuloevyr));
	...
	PUHD_PO = MAX(SUM(POTULOT, -YRVAHP, -vulthanmu, -tjmarkh, -vohvah), 0);
	...
	/* Ansiotulovähennys (-2024) */
	IF &LVUOSI >= 2023 AND SUM(PALKKA1, MUU_TYO, YRITYSTA, -YRVAHA, OSINKOA) > 0 THEN DO;
		%AnsVahS(ANSIOT_VAH, &LVUOSI, &INF, PUHD_ANSIO, SUM(PALKKA1, MUU_TYO, YRITYSTA, -YRVAHA, OSINKOA));
	END;
	ELSE IF &LVUOSI <= 2022 AND SUM(PALKKA1, MUU_TYO, YRITYSTA, -YRVAHA, IFN(&LVUOSI > 2004, OSINKOA, 0)) > 0 THEN DO;
	...
	/* Eläketulovähennys (2023-) */
	IF ELAKE > 0 THEN DO;
		%ElTulVahS(ELTULVAH, &LVUOSI, &INF, ELAKE, PUHD_ANSIO);
	END;
	ELSE ELTULVAH = 0;

	/* Kunnallisveron eläketulovähennys (-2022) */
	IF ELAKE > 0 THEN DO;
		%KunnElTulVahS(ELTULVAH_K, &LVUOSI, &INF,  PSOT, 0, ELAKE, PUHD_ANSIO, 0);
	END;
	ELSE ELTULVAH_K = 0;

	/* Valtioverotuksen eläketulovähennys (-2022) */
	IF elake > 0 THEN DO;
	...
	/* Opintorahavähennys */
	IF &LVUOSI >= 2023 AND OPTUKI_SIMUL > 0 THEN DO;
		%OpRahVahS(OPRAHVAH, &LVUOSI, &INF, OPTUKI_SIMUL, PUHD_ANSIO);
	END;
	ELSE IF &LVUOSI <= 2022 AND OPTUKI_SIMUL > 0 THEN DO;
		%KunnOpRahVahS(OPRAHVAH, &LVUOSI, &INF, 1, OPTUKI_SIMUL, SUM(ANSIOT, -YRVAHA), PUHD_ANSIO);
	END;
	ELSE OPRAHVAH = 0;

	/* Merityötulovähennys (2023-) */
	IF tmerile > 0 THEN DO;
		%VerMeriVahS(MERIVAH, &LVUOSI, &INF, tmerile);
	END;
	ELSE MERIVAH = 0;

	/* Kunnallisverotuksen merityötulovähennys (-2022) */
	IF tmerile > 0 THEN DO;
		%KunnVerMeriVahS(MERIVAHKUN, &LVUOSI, &INF, tmerile);
	END;
	ELSE MERIVAHKUN = 0;

	/* Valtionverotuksen merityötulovähennys (-2022) */
	IF tmerile > 0 THEN DO;
		%ValtVerMeriVahS(MERIVAHVAL, &LVUOSI, &INF, tmerile); 
	END;
	ELSE MERIVAHVAL = 0;

	/* Kunnallisverotuksen invalidivähennys (-2022) */
	IF invpros > 0 OR invah82 > 0 THEN DO;
		%KunnVerInvVahS(INVVAH_K, &LVUOSI, &INF, IFN(invah82 > 0, 1, 0), IFN(invah82 > invpros, invah82, invpros), PUHD_ANSIO, ELAKE);
	END;
	ELSE INVVAH_K = 0;

	/* Valtionverotuksen invalidivähennys (-2022) */
	IF invpros > 0 THEN DO;
		%ValtVerInvVahS(INVVAH_V, &LVUOSI, &INF, invpros, ELAKE);
	END;
	ELSE INVVAH_V = 0;
				
	/* Verotettava tulo ennen perusvähennystä (2023-) */
	IF &LVUOSI >= 2023 THEN
		VERTULO1 = MAX(SUM(ANSIOT, -YRVAHA, -PALKVAK, -PRAHAMAKSU, -THANKKULUT2, -ELTULVAH, -ANSIOT_VAH, -MERIVAH, -OPRAHVAH, -MUU_VAH_VER), 0);
	ELSE VERTULO1 = 0;
	/* Perusvähennys (2023-) */
	IF VERTULO1 > 0 THEN DO;
		%PerVahS(PERVAH, &LVUOSI, &INF, VERTULO1);
	END;
	ELSE PERVAH = 0;
	/* Verotettava tulo perusvähennyksen jälkeen (2023-) */
	VERTULO2 = MAX(SUM(VERTULO1, -PERVAH), 0); 

	/* Kunnallisverotuksessa verotettava tulo ennen perusvähennystä (-2022) */
	IF &LVUOSI <= 2022 THEN
	...
	ELSE KUNNVTULO1 = 0;
	/* Kunnallisverotuksen perusvähennys (-2022) */
	IF KUNNVTULO1 > 0 THEN DO;
		%KunnPerVahS(PERVAH_K, &LVUOSI, &INF, KUNNVTULO1);
	END;
	ELSE PERVAH_K = 0;
	/* Kunnallisverotuksessa verotettava tulo perusvähennyksen jälkeen (-2022) */
	KUNNVTULO2 = MAX(SUM(KUNNVTULO1, -PERVAH_K), 0);

	IF &LVUOSI >= 2023 THEN VTULO1 = VERTULO2;
	ELSE VTULO1 = KUNNVTULO2;

	/* Kunnallis- ja kirkollisveroja */
	KUNNVEROA = &kunnkerroin * 0.01 * AYRI * VTULO1 / 100;
	KIRKVEROA = &kirkkerroin * 0.01 * KAYRI * VTULO1 / 100;

	/* Käänteinen päättely sairausvakuutusmaksuihin (2023-) */ 
	IF &LVUOSI >=2023 AND &TARKPVM = 1 AND YRITTAJA = 1 THEN DO;
		PUHD_ANSIOSV = SUM(PUHD_ANSIO, -SUM(YRITYSTA, -YRVAHA), PRAHAMAKSUTULO);
		/* Eläketulovähennys */
		%ElTulVahS (ELTULVAH_SV, &LVUOSI, &INF, ELAKE, PUHD_ANSIOSV);
		/* Ansiotulovähennys (2023-2024) */
		%AnsVahS(ANSIOT_VAHSV, &LVUOSI, &INF, PUHD_ANSIOSV, PRAHAMAKSUTULO);
		/* Opintorahavähennys */
		%OpRahVahS(OPRAHVAHSV, &LVUOSI, &INF, OPTUKI_SIMUL, PUHD_ANSIOSV);
		VTULO1SV = MAX(SUM(ANSIOT, -YRITYSTA, PRAHAMAKSUTULO, -PALKVAK, -PRAHAMAKSU, -THANKKULUT2, -ELTULVAH_SV, -ANSIOT_VAHSV, -OPRAHVAHSV, -MUU_VAH_VER), 0);
		/* Perusvähennys */
		%PerVahS(PERVAHSV, &LVUOSI, &INF, VTULO1SV);
		VTULO2SV = MAX(SUM(VTULO1SV, -PERVAHSV), 0);
		/* Sairausvakuutusmaksu */
		%SairVakMaksuS(SAIRVAKA, &LVUOSI, &INF, VTULO2SV, ELAKE, PRAHAMAKSUTULO);
	END;
	/* Käänteinen päättely sairausvakuutusmaksuihin (-2022) */ 
	ELSE IF &LVUOSI <= 2022 AND &TARKPVM = 1 AND YRITTAJA = 1 THEN DO;
		PUHD_ANSIOSV = SUM(PUHD_ANSIO, -SUM(YRITYSTA, -YRVAHA), PRAHAMAKSUTULO);
		...
		%KunnVerInvVahS(INVVAH_KSV, &LVUOSI, &INF, IFN(invah82 > 0, 1, 0), IFN(invah82 > invpros, invah82, invpros), PUHD_ANSIOSV, ELAKE);
	...
	ELSE DO;
		...
		IF VTULO1 > 0 THEN DO;	
			%SairVakMaksuS(SAIRVAKA, &LVUOSI, &INF, VTULO1, ELAKE, PRAHAMAKSUTULO);
		END;
		ELSE SAIRVAKA = 0;
	END;

	/* Kansaneläkevakuutusmaksu */
	IF MAX(SUM(VTULO1, -upuhdateisv), 0) > 0 THEN DO;	
		%KansElVakMaksuS(KEVA, &LVUOSI, &INF, MAX(SUM(VTULO1, -upuhdateisv), 0), ELAKE);
	END;
	ELSE KEVA = 0;

	/* Valtionverotuksessa verotettava tulo */
	IF &LVUOSI >= 2023 THEN 
		VTULO2 = VERTULO2;
	ELSE DO;
	...
	/* Valtion tulovero */
	IF VTULO2 > 0 THEN DO;
		%ValtTuloVeroS(VALTVEROA, &LVUOSI, &INF, VTULO2, maakunta);
	END;
	ELSE VALTVEROA = 0;

	/* Eläketulon lisävero, lisätään valtion tuloveroon (2013-) */
	IF &LVUOSI >= 2023 AND ELAKE > 0 THEN DO;
		%ElakeLisaVeroS(ELAKELISAVERO, &LVUOSI, &INF, ELAKE, ELTULVAH);
	END;
	ELSE IF &LVUOSI <= 2022 AND ELAKE > 0 THEN DO;
	...
	IF SUM(PALKKA1, MUU_TYO, YRITYSTA, -YRVAHA, IFN(&LVUOSI > 2004, OSINKOA, 0)) > 0 THEN DO;
		%TyotVahS(TYOTULVAH, &LVUOSI, &INF, SUM(PALKKA1, MUU_TYO, YRITYSTA, -YRVAHA, IFN(&LVUOSI > 2004, OSINKOA, 0)), PUHD_ANSIO, ikavu, YKSINHUOLTAJA, cllkm);
	END;
	ELSE TYOTULVAH = 0;

	/* Valtionverotuksen elatusvelvollisuusvähennys */
	IF evlaplkm > 0 THEN DO;
		%ValtVerElVelvVahS(ELVELV_VAH, &LVUOSI, &INF, evlaplkm, velatk);
	END;
	ELSE ELVELV_VAH = 0;
	...
	IF &LVUOSI >= 2022 AND SUM(palksiku_yht, palkomos_yht, tyonosuu_yht) > 0 THEN DO;
		%KotitVahErillJakoS(KOTITVAH, &LVUOSI, &INF, palksiku_remontti, palksiku_koti, palksiku_oljy, palkomos_remontti, palkomos_koti, palkomos_oljy, tyonosuu_remontti, tyonosuu_koti, tyonosuu_oljy);
	END;
	/* Kotitalousvähennys (-2021) */
	ELSE IF &LVUOSI <=2021 AND SUM(palksiku_yht, palkomos_yht, tyonosuu_yht) > 0 THEN DO;
		%KotitVahErillS(KOTITVAH, &LVUOSI, &INF, palksiku_yht, palkomos_yht, tyonosuu_yht);
	END;
	ELSE KOTITVAH = 0;

	/* Lapsivähennys (2015-2017) */
	...
	/* Työtulovähennys (ent. valtionverotuksen ansiotulovähennys) */
	...
	ennankonpidätyksenä (lopullisten verojen vähennyksenä). Vuodesta 2009 lähtien vähennyksen nimi on ollut työtulovähennys ja se vähennetään ensin valtion 
	...
	IF &LVUOSI < 2009 THEN DO;
		VALTVEROB = MAX(SUM(VALTVEROA, -TYOTULVAH), 0);
		ANVAHYLIJ = MAX(SUM(TYOTULVAH, -VALTVEROA), 0);
		KUNNVEROB = KUNNVEROA;
		SAIRVAKB = SAIRVAKA;
		KEVB = KEVA;
		KIRKVEROB = KIRKVEROA;
	END;
	ELSE DO;
		IF TYOTULVAH > 0 THEN DO;
		    %VahennJakoS(VALTVEROB, &LVUOSI, TYOTULVAH, 1, VALTVEROA, KUNNVEROA, SAIRVAKA, KEVA, KIRKVEROA, 0); /* valt. ans. vero */
			%VahennJakoS(KUNNVEROB, &LVUOSI, TYOTULVAH, 2, VALTVEROA, KUNNVEROA, SAIRVAKA, KEVA, KIRKVEROA, 0); /* kunnallisvero */
			%VahennJakoS(SAIRVAKB, &LVUOSI, TYOTULVAH, 3, VALTVEROA, KUNNVEROA, SAIRVAKA, KEVA, KIRKVEROA, 0); /* svak-maksu */
			%VahennJakoS(KEVB, &LVUOSI, TYOTULVAH, 4, VALTVEROA, KUNNVEROA, SAIRVAKA, KEVA, KIRKVEROA, 0); /* kev-maksu */
			%VahennJakoS(KIRKVEROB, &LVUOSI, TYOTULVAH, 5, VALTVEROA, KUNNVEROA, SAIRVAKA, KEVA, KIRKVEROA, 0); /* kirkollisvero */
		END;
		ELSE DO;
			VALTVEROB = VALTVEROA;
			KUNNVEROB = KUNNVEROA;
	...
	IF VEROPUOL = 0 THEN DO;
		VALTVEROD = MAX(SUM(VALTVEROC, -INVVAH_V, -ELVELV_VAH), 0);
	...
	ANSIOVEROT = SUM(VALTVEROH, KUNNVEROG, SAIRVAKG, KEVG, KIRKVEROG, -ANVAHYLIJ);
		...
		KEEP hnro OSINKOP1 OSINKOP2 OSINKOP3 OSINKOVAP1 OSINKOVAP2 OSINKOVAP3 OSINKOVAP4
			 ...
			 THANKK MUU_VAH_VALT2 MUU_VAH_KUNN2 MUU_VAH_VER POTAPP ULK_OSUUS
			 ...
			 PUHD_ANSIO PUHD_PO KOTITVAH_DATA ANSIOT_VAH ELTULVAH ELTULVAH_K ELTULVAH_V
			 OPRAHVAH INVVAH_K PALKVAK PRAHAMAKSU PERVAH KUNNVTULO1 KUNNVTULO2 PERVAH_K
			 KUNNVEROA KUNNVEROB KUNNVEROC KUNNVEROD KUNNVEROE KUNNVEROF KUNNVEROG 
			 ...
			 VALTVEROA VALTVEROB VALTVEROC VALTVEROD VALTVEROE VALTVEROF VALTVEROG VALTVEROH TYOTULVAH
			 ...
			 ASKOROT ENSASKOROT LAPSIVAH MERIVAH MERIVAHKUN MERIVAHVAL YRVAHA YRVAHP VERTULO1 VERTULO2;
...
ANSIOT_VAH = 'Ansiotulovähennys (2023-2024), MALLI'
ELTULVAH = 'Eläketulovähennys (2023-), MALLI'
ELTULVAH_K = 'Kunnallisverotuksen eläketulovähennys (-2022), MALLI'
ELTULVAH_V = 'Valtionverotuksen eläketulovähennys (-2022), MALLI'
OPRAHVAH = 'Opintorahavähennys, MALLI'
INVVAH_K = 'Kunnallisverotuksen invalidivähennys (-2022), MALLI'
INVVAH_V = 'Valtionverotuksen invalidivähennys (-2022), MALLI'
MERIVAH = 'Merityötulovähennys (2023-), MALLI'
MERIVAHKUN = 'Kunnallisverotuksen merityötulovähennys (-2022), MALLI'
MERIVAHVAL = 'Valtionverotuksen merityötulovähennys (-2022), MALLI'
PALKVAK = 'Palkansaajan eläke- ja työttömyysvakuutusmaksut yhteensä, MALLI'
PRAHAMAKSU = 'Sairausvakuutuksen päivärahamaksu, MALLI'
KUNNVTULO1 = 'Kunnallisverotuksessa verotettava tulo ennen perusvähennystä (-2022), MALLI'
PERVAH_K = 'Kunnallisverotuksen perusvähennys (-2022), MALLI'
KUNNVTULO2 = 'Kunnallisverotuksessa verotettava tulo perusvähennyksen jälkeen (-2022), MALLI'
VERTULO1 = 'Verotettava tulo ennen perusvähennystä (2023-), MALLI'
PERVAH = 'Perusvähennys (2023-), MALLI'
VERTULO2 = 'Verotettava tulo perusvähennyksen jälkeen (2023-), MALLI'
KUNNVEROA = 'Kunnallisvero, vaihe 1: ennen vähennyksiä, MALLI'
KUNNVEROB = 'Kunnallisvero, vaihe 2: työtulovähennyksen jälkeen, MALLI'
...
KIRKVEROB = 'Kirkollisvero, vaihe 2: työtulovähennyksen jälkeen, MALLI'
...
SAIRVAKB = 'Sairaanhoitomaksu, vaihe 2: työtulovähennyksen jälkeen, MALLI'
...
KEVB = 'Kansaneläkevakuutusmaksu, vaihe 2: työtulovähennyksen jälkeen, MALLI'
...
VALTVEROB = 'Valtion tulovero, vaihe 2: työtulovähennyksen jälkeen, MALLI'
VALTVEROC = 'Valtion tulovero, vaihe 3: lapsivähennys, MALLI'
VALTVEROD = 'Valtion tulovero, vaihe 4: elatusvelvollisuusvähennyksen jälkeen, MALLI'
...
TYOTULVAH = 'Työtulovähennys (ent. valtionverotuksen ansiotulovähennys), MALLI'
