/******************************************************
* Kuvaus: Kotihoidon tuen simulointimalli	          *
*******************************************************/ 
...
%IF &START NE 1 %THEN %DO;
	...
	* Tulostaulukoiden esivalinnat ; 
	...
	%LET MUUTTUJAT =  tkotihtu lgkthr HOITORAHA lgkthl HOITOLISA KOTIHTUKI_DATA KOTIHTUKI lgos OSHOIT lgjhhr JSHOIT ; * Taulukoitavat muuttujat (summataulukot) ; 
...
%IF &POIMINTA = 1 %THEN %DO; 
	...
	* Summataan tarvittavia muuttujia kotitaloustasolle ;
	...
	WHERE = (lgkthr > 0 OR lgkthl > 0 OR lgkk > 0 OR lgos > 0 OR lgjhhr > 0
	OR hltulo >= 0 OR hlkk NE . OR  la1 NE . OR la2 NE . OR la3 NE . OR la4 NE . OR la5 NE . OR 
	la6 NE . OR la7 NE . OR la8 NE . OR la9 NE .)) 
	SUM(lgkthr lgkthl tkotihtu lgkk lgos lgjhhr hltulo hlkk la1 la2 la3 la4 la5 la6 la7 la8 la9 
	...
	* Poimitaan aineiston tiedot eri tukilajeista kotitalouden sis‰ist‰ jakoa varten;
	...
	(KEEP = hnro knro lgkthr lgkthl); 
	WHERE SUM(lgkthr, lgkthl) > 0;
	KOTIHTU = SUM(lgkthr, lgkthl);
	RUN;

	DATA STARTDAT.START_KOTIHTUKI_lgos;
	SET POHJADAT.&AINEISTO&AVUOSI
	(KEEP = hnro knro lgos ostukikuuk); 
	WHERE lgos > 0; 
	RUN;
...
* Lasketaan kotitalouksien eri henkilˆille suhteellinen osuus osittaisesta hoitorahasta ja tukikuukausista: 
  lasketaan ensin summa kotitalouksittain tiedostoon KOTIHTUKI_lgosx ;

PROC SUMMARY DATA = STARTDAT.START_KOTIHTUKI_lgos;
BY knro; 
WHERE (lgos > 0);
OUTPUT OUT = TEMP.KOTIHTUKI_lgosx (DROP = _TYPE_ _FREQ_) SUM(lgos) = lgos_SUM SUM(ostukikuuk) = ostukikuuk_SUM; 
RUN;

* Lasketaan henkilˆiden osuudet osittaisesta hoitorahasta, tiedosto KOTIHTUKI_HENK_OSUUDET_lgos ja muuttujat OSUUS_lgos OSUUS_ostukikuuk;

DATA TEMP.KOTIHTUKI_HENK_OSUUDET_lgos;
MERGE STARTDAT.START_KOTIHTUKI_lgos TEMP.KOTIHTUKI_lgosx (KEEP = knro lgos_SUM ostukikuuk_SUM); 
BY knro;
OSUUS_lgos = SUM(lgos) / SUM(lgos_SUM);
...
* Lasketaan henkilˆiden osuudet osittaisesta hoitorahasta, tiedosto KOTIHTUKI_HENK_OSUUDET_lgos ja muuttujat OSUUS_lgjhhr OSUUS_jstukikuuk;
...
MERGE TEMP.KOTIHTUKI_HENK_OSUUDET_kotihtu(KEEP = hnro knro OSUUS)
	TEMP.KOTIHTUKI_HENK_OSUUDET_lgos(KEEP = hnro knro OSUUS_lgos OSUUS_ostukikuuk ostukikuuk ostukikuuk_SUM)
...
DATA TEMP.&TULOSNIMI_KT;
MERGE TEMP.&TULOSNIMI_KT(KEEP = hnro knro HOITORAHA HOITOLISA TUKI OSHOIT JSHOIT lgos lgjhhr tkotihtu) 
...
OSUUS_lgos = 'Henkilˆn osuus osittaisesta hoitorahasta, MALLI'
...
KEEP knro hnro HOITORAHA HOITOLISA KOTIHTUKI OSHOIT JSHOIT OSUUS OSUUS_lgos OSUUS_ostukikuuk OSUUS_lgjhhr OSUUS_jstukikuuk;
...
%IF &START NE 1 %THEN %DO;
	...
	%IF &TULOSLAAJ = 1 %THEN %DO; 
		...
		(KEEP = hnro knro &PAINO tkotihtu lgkthr lgkthl lgos lgjhhr ikavu ikavuv desmod soss paasoss elivtu koulas koulasv rake maakunta)
		TEMP.&TULOSNIMI_KT;
	%END;
	...
	KOTIHTUKI_DATA = SUM(lgkthr, lgkthl);

	* Asetetaan muuttujien 0-arvot tyhjiksi, jotta lukum‰‰r‰t voidaan laskea suoraan ;

	ARRAY PISTE 
	tkotihtu lgkthr lgkthl KOTIHTUKI_DATA lgos lgjhhr; 
	DO OVER PISTE;
		IF PISTE <= 0 THEN PISTE = .;
	END;
	...
	* Annetaan datan muuttujille selitteet;
	...
	lgkthr = 'Lasten kotihoidon tuen hoitoraha, DATA'
	lgkthl = 'Lasten kotihoidon tuen hoitolis‰, DATA'
	KOTIHTUKI_DATA = 'Lasten kotihoidon tuki, DATA'
	lgos = 'Osittainen hoitoraha, DATA'
