...
*Makron parametrit:
	...
	ikavu: Ikä, vuosia
	svpalkka: Työnantajamaksujen pohjalla olevat palkkatulot
	pros: Työnantajan keskimääräinen eläkevakuutusmaksuprosentti (KuEl, VaEl)
	sektori:Työnantajasektori (1=yritykset, 2=rah. ja vak.laitokset, 3=kunta, kuntien liikelaitokset,
		4=voittoa tavoittelemattomat, 5=kotitaloudet, 6=ulkomaat, 8=valtio ja sos rahastot, 9=as oy);

%MACRO ElMaksuKTAS (tulos, mvuosi, mkuuk, inf, ikavu, svpalkka, pros, sektori)/
...
IF &mvuosi LE 2016 AND 18 LE &ikavu LE 67 AND &svpalkka GT 0 THEN DO;
		...
		IF &pros NE . THEN &tulos = (&pros/100) * &svpalkka;
		ELSE IF &sektori EQ 3 THEN &tulos = (&KuEL/100) * &svpalkka;
...
%MACRO ElMaksuVTAS (tulos, mvuosi, inf, ikavu, svpalkka, pros, sektori)/
DES = 'TAMAKSU: Työnantajan eläkevakuutusmaksu kuukausitasolla vuosikeskiarvona';

elmaksuV = 0;

%DO i = 1 %TO 12; 
 	%ElMaksuKTAS (temp, &mvuosi, &i, &inf, &ikavu, &svpalkka, &pros, &sektori);
