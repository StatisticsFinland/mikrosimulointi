...
%IF &START NE 1 %THEN %DO;
	...
	%LET VARALLISUUSDATA = 0; * Käytetäänkö ASUMTUKI-mallissa datan varallisuustietoja (1 = Kyllä, 0 = Ei). 
	...
	* Tulostaulukoiden esivalinnat ; 
	...
	%LET MUUTTUJAT = TUKIVUOK TUKIOMOS TUKIOMTA TUKIOM TUKIOSA TUKISUMMA yastuki; * Taulukoitavat muuttujat (summataulukot) ;
	...
	* Laskettavat tunnusluvut (jos tyhjä, niin ei lasketa);
	...
	/* Tarkistetaan onko ELASUMTUKI- ja OPINTUKIsimul ajettuna (kyllä/ei (1/0)) */
...
* Annetaan ERROR jos käyttäjä on valinnut lainsäädäntövuodelle 2017 tai 2025 simulointityypin SIMUL;
%IF (&LVUOSI = 2017 OR &LVUOSI = 2025) AND %UPCASE(&TYYPPI) = SIMUL %THEN %DO;
	%PUT ERROR: Lainsäädäntövuodelle &LVUOSI ei voi käyttää ASUMTUKI-mallissa simulointityyppiä SIMUL;
	%PUT ERROR: Valitse TYYPPI = SIMULX ja haluamasi lainsäädäntökuukausi (LKUUK)!;
	%ABORT CANCEL;
%END;

*Tarkistetaan onko ASUMTUKIsimul-osamallin simulointiin tarvittavat ELASUMTUKI- ja OPINTUKIsimul ajettu;

%IF &TARKISTUS_ASUMTUKI = 1 %THEN %DO;

	%Tarkistus(STARTDAT.start_elasumtuki_henki, ELASUMTUKIsimul, 7); 

	%IF (&LVUOSI <= 2017 OR &LVUOSI >= 2025) %THEN %DO;
		%Tarkistus(TEMP.&TULOSNIMI_OT, OPINTUKIsimul, 7); 
	%END;
...
%IF &POIMINTA = 1 %THEN %DO;
	...
	DATA STARTDAT.START_ASUMTUKI_HENKI;
		...
		telps2 telps5 ttyoltuk tutmp235 tutmp4 tmtatt tpjta tyhtat
		tmaat1evyr tmaat1pevyr tliik1evyr tliikpevyr tporo1evyr 
		ymaatattuloevyr elyelattuloevyr ymaatattulo elyelattulo yrvah
		psiraho pulkyso karvo posake_arvo_yht_AOT mtnetvaosvv elyosnetvvv pliikpos pmaatpos valopullinenptd tpalk5 tulmuuh tulmuuv tulmuusu);
	RUN;
	...
	/* Yhdistetään jyvitetyt tiedot taulukkoon START_ASUMTUKI_HENKI */
	DATA STARTDAT.START_ASUMTUKI_HENKI;
	...
	/* Käytetään OPINTUKI-mallissa tuotettua tulostaulua, jonka avulla rajataan pois 
	opintotuen asumislisää saaneet henkilöt */
	%IF (&LVUOSI <= 2017 OR &LVUOSI >= 2025) %THEN %DO;
		PROC SQL;
			DELETE FROM STARTDAT.START_ASUMTUKI_HENKI
			WHERE hnro IN (SELECT DISTINCT hnro FROM TEMP.&TULOSNIMI_OT WHERE ASUMLISA > 0);
		QUIT;
	%END;
	...
	DATA STARTDAT.START_ASUMTUKI_HENKI;
		...
		TYOTULOKK = SUM(tepalkat, toptiot, tosinktp, tulkp, telps43, tmuukust, tepalk,
			...
			ttyoltuk, tutmp235, tutmp4, tmaat1evyr, tmaat1pevyr, tpjta, tliik1evyr, 
			tliikpevyr, tporo1evyr, ymaatattuloevyr, elyelattuloevyr,
			SUM(tyhtat, -ymaatattulo, -elyelattulo), tmtatt, tpalk5, tulmuuh, tulmuuv, tulmuusu) / 12;
	...
	PROC SQL;
	    CREATE TABLE TEMP.TEMP_ASUMTUKI_KOTI1 AS
	    SELECT DISTINCT
	        knro,
	        elivtu,
	        yastukikr,
	...
	PROC SQL;
	    CREATE TABLE TEMP.TEMP_ASUMTUKI_KOTI2 AS
	    SELECT 
	        knro AS knro2,
	        SUM(svatva) AS svatva,
	        SUM(svatvp) AS svatvp,
	        SUM(maksvuok) AS maksvuok,
	        SUM(hoitvast) AS hoitvast,
	        ...
	        SUM(odorkeko) AS odorkeko,
	        SUM(tnoosvvb) AS tnoosvvb,
	        SUM(teinovvb) AS teinovvb,
	        SUM(tuosvvap) AS tuosvvap,
	        SUM(toyjmyvvap) AS toyjmyvvap,
	        SUM(toyjmavvap) AS toyjmavvap,
	        SUM(teinovv) AS teinovv,
	        SUM(yastuki) AS yastuki,
	        SUM(yrvah) AS yrvah,
			sum(psiraho) AS psiraho,
			sum(pulkyso) AS pulkyso,
			sum(karvo) AS karvo,
			sum(posake_arvo_yht_AOT) AS posake_arvo_yht_AOT,
			sum(mtnetvaosvv) AS mtnetvaosvv,
			sum(elyosnetvvv) AS elyosnetvvv,
			sum(pliikpos) AS pliikpos,
			sum(pmaatpos) AS pmaatpos,
			sum(valopullinenptd) AS valopullinenptd,
	        COUNT(hnro) AS HENKIL,
	        MIN(hnro) AS hnro
	    FROM STARTDAT.START_ASUMTUKI_HENKI
	    GROUP BY knro
	;
	...
	PROC SQL;
	    CREATE TABLE TEMP.TEMP_ASUMTUKI_KOTI3 AS
	    SELECT 
	        knro AS knro3,
	        SUM(svatva) AS svatva18,
	        SUM(svatvp) AS svatvp18,
	        SUM(odorsyke) AS odorsyke18,
	        SUM(odorkeke) AS odorkeke18,
			SUM(odorsyko) AS odorsyko18,
	        SUM(odorkeko) AS odorkeko18,
	        SUM(tnoosvvb) AS tnoosvvb18,
	        SUM(teinovvb) AS teinovvb18,
	        SUM(tuosvvap) AS tuosvvap18,
	        SUM(toyjmyvvap) AS toyjmyvvap18,
	        SUM(toyjmavvap) AS toyjmavvap18,
	        SUM(teinovv) AS teinovv18,
	        SUM(yrvah) AS yrvah18,
	        COUNT(hnro) AS AIK
	    FROM STARTDAT.START_ASUMTUKI_HENKI
			WHERE ikavu >= 18
	    GROUP BY knro
	...
	PROC SQL;
		CREATE TABLE TEMP.TEMP_ASUMTUKI_KOTI4(drop=knro2 knro3) AS 
		SELECT 
			a.*,
			b.*, 
			c.*
		FROM TEMP.TEMP_ASUMTUKI_KOTI1 AS a
			LEFT JOIN TEMP.TEMP_ASUMTUKI_KOTI2 AS b ON a.knro = b.knro2
			LEFT JOIN TEMP.TEMP_ASUMTUKI_KOTI3 AS c ON a.knro = c.knro3
	;
	QUIT;
	...
	/* Käännetään TYOTULOKK muuttujat sarakkeille kotitalouksittain ja liitetään se kotitaloustason taulukkoon */
	...
	ARRAY TUL{13} svatva18 svatvp18 odorsyke18 odorkeke18 odorsyko18 odorkeko18 tnoosvvb18 teinovvb18 tuosvvap18 toyjmyvvap18 toyjmavvap18 teinovv18 yrvah18;
	DO I=1 TO 13;
	...
	OSVEROVAP_DATA18 = SUM(tnoosvvb18, teinovvb18, tuosvvap18, toyjmyvvap18, toyjmavvap18, teinovv18);
   	KUUKTULO_DATA18 = MAX(SUM(svatva18, svatvp18, yrvah18, -odorsyke18, -odorkeke18, -odorsyko18, -odorkeko18, OSVEROVAP_DATA18) / 12, 0);
	OPTUKI_DATA18 = MAX(SUM(odorsyke18, odorkeke18, odorsyko18, odorkeko18) / 12, 0);
	...
	IF &VARALLISUUSDATA = 1 
		THEN VARALLISUUS_DATA = SUM(psiraho, pulkyso, karvo*0.7, posake_arvo_yht_AOT*0.7, mtnetvaosvv, elyosnetvvv, pliikpos, pmaatpos, valopullinenptd);
		ELSE VARALLISUUS_DATA = 0;

	KEEP knro OMALAMM OMAMAKS ASLAIKOR KUUKTULO_DATA OPTUKI_DATA maksvuok VUOKRA_AS KESKLAMM 
		 aslaji OMAKOTI YKSH LAMMR yastukikr astukikr_l15 astukikr_l24 astukikr_l25 HENKIL rakvuosi halpinta hoitvast maksvuok
		 yastuki AIK TYOTULOKK1-TYOTULOKK%EVAL(&AIKMAX) KUUKTULO_DATA18 OPTUKI_DATA18 LKRYHMA KUNTARYHMA VARALLISUUS_DATA;  
	...
	KUNTARYHMA = 'Asumistuen kuntaryhmä, DATA'
...
%ELSE %DO;
	%IF &LVUOSI < 2015 %THEN %DO; 
		OPINTUKI = OPTUKI_DATA * 12;
		OPINTUKI_HUOM = OPTUKI_DATA;
	%END;
	%ELSE %DO;
		OPINTUKI = OPTUKI_DATA18 * 12;
		OPINTUKI_HUOM = OPTUKI_DATA18;
	%END;
%END;

%IF &VERO = 1 %THEN %DO;
	%IF &LVUOSI < 2015 %THEN %DO;
		KUUKTULO = SUM(ANSIOT, POTULOT, -OPINTUKI, OSINKOVAP) / 12;
	%END;
	%ELSE %DO;
		KUUKTULO = SUM(ANSIOT18, POTULOT18, -OPINTUKI, OSINKOVAP18) / 12;
	%END;
%END;
%ELSE %DO;
	%IF &LVUOSI < 2015 %THEN %DO; 
...
%IF &LVUOSI < 2015 %THEN %DO;
	...
	* Asumistuki vuokra-asunnoissa ;
	IF VUOKRA_AS NE 0 and aslaji NE 5 THEN DO;
		...
		rakvuosi, halpinta, OMAVAST, maksvuok, OMAMAKS, OMALAMM);
	END;
	* Asumistuki omistusasunnoissa ;
	IF VUOKRA_AS = 0 THEN DO;
		...
		rakvuosi, halpinta, OMAVAST, hoitvast, OMAMAKS, OMALAMM, ASLAIKOR, 0);
	END;
...
%ELSE %DO;
	...
	* Asumistuki vuokra-asunnossa ;
	IF maksvuok > 0 AND aslaji NE 6 THEN DO;
		%As2015AsumistukiVuokraVS(TUKIVUOK, &LVUOSI, &INF, KUNTARYHMA, 0, maksvuok, 0, (OMAMAKS > 0), (OMALAMM > 0), LKRYHMA, KUUKTULO, TYOTULO_ARRAY, AIK, SUM(HENKIL, -AIK), VARALLISUUS_DATA, opinraha=OPINTUKI_HUOM);
	END;
	* Asumistuki omassa asunto-osakeasunnossa ;
	ELSE IF aslaji = 3 OR (aslaji = 6 AND maksvuok > 0) THEN DO;
		%As2015AsumistukiOmaOsakeVS(TUKIOMOS, &LVUOSI, &INF, KUNTARYHMA, 0, SUM(hoitvast, maksvuok), 0, (OMAMAKS > 0), (OMALAMM > 0), LKRYHMA, ASLAIKOR, KUUKTULO, TYOTULO_ARRAY, AIK, SUM(HENKIL, -AIK), VARALLISUUS_DATA, opinraha=OPINTUKI_HUOM);
	END;
	* Asumistuki omassa omakotitalossa ;
	ELSE IF aslaji = 1 OR aslaji = 2 THEN DO;
		%As2015AsumistukiOmaTaloVS(TUKIOMTA, &LVUOSI, &INF, KUNTARYHMA, 0, 0, LKRYHMA, ASLAIKOR, KUUKTULO, TYOTULO_ARRAY, AIK, SUM(HENKIL, -AIK), VARALLISUUS_DATA, opinraha=OPINTUKI_HUOM);
	END;
	TUKIVUOK = 12 * TUKIVUOK;
...
%IF &START NE 1 %THEN %DO;
	...
	%IF &TULOSLAAJ = 1 %THEN %DO; 
		...
		(KEEP = hnro knro &PAINO yastuki ikavuv desmod paasoss elivtu koulas koulasv rake maakunta)
		TEMP.&TULOSNIMI_YA;
	%END;
	...
	* Luodaan datan muuttujille selitteet ;
	...
	LABEL 
	yastuki = 'Yleinen asumistuki yhteensä, DATA'; 
