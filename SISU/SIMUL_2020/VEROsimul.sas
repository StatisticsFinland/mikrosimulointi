/*************************************************************
*  Kuvaus: Tuloverotuksen simulointimalli 2019		         * 
*  Viimeksi päivitetty: 13.4.2023 MK	  					 * 
* ***********************************************************/
	...
	* Inflaatiokorjaus. Euro- tai markkamääräisten parametrien haun yhteydessä suoritettavassa
	...
	%IF &LVUOSI<2023 %THEN %DO; 
		%LET MUUTTUJAT = ANSIOT POTULOT KOKONTULO ltva VALTVEROH ltvp POVEROC lkuve KUNNVEROG lkive KIRKVEROG lshma SAIRVAKG lelvak PALKVAK 
					 ...
					 OSINKOA_DATA OSINKOA OSINKOP_DATA OSINKOP OSINKOVAP_DATA OSINKOVAP; /* Taulukoitavat muuttujat, vanha lainsäädäntö (summataulukot) */
	%END;
...
%IF &POIMINTA = 1 %THEN %DO;
	...
	SET POHJADAT.&AINEISTO&AVUOSI (KEEP = 
	 ...
	 vvevah vvvmk1 vvvmk3 vvvmk5 yrtukor hsaiprva haiprva tkotihtu
	 hwmky tkoultuk tmtukimk tkopira ktku htkapr hkotihm takuuel oplaikor
	 tjmarkh tvahep50 tptvs tvahep20 tptsu50 korosatkg
	 dtyhtep korosapkw elivtu lylen tkapite tsijova
	 palksiku_remontti palksiku_koti palkomos_remontti palkomos_koti tyonosuu_remontti tyonosuu_koti
	 vthmkor palksiku_yht palkomos_yht tyonosuu_yht 
	 tosmetpt tulkelvp tyhtmat tyhtate tyhtmpot tyhtpote
	...
	/* Vuoden aineistossa äyrit myös kahdelle seuraavalle vuodelle */
	%IF ((&LVUOSI >= 2020) AND (&LVUOSI <= 2022)) %THEN %DO; 
	...
	TTURVA_DATA = SUM(MAX(vvvmk1, 0), MAX(vvvmk3, 0), MAX(vvvmk5, 0),
					MAX(0, SUM(dtyhtep, korosapkw)),
				    MAX(SUM(tmtukimk, korosatkg), 0));
	...
	MUU_ANSIO = SUM(tlakko, tpalv2a, tomlis, telvpal, tsuurpu, MUU_TTURVA_DATA, tapur);
	THANKK = SUM(vthm4);
	MUU_VAH_VALT2 = SUM(vmuut1, vmuutv, MAX(SUM(vevm, -lelvak),0));
	MUU_VAH_KUNN2 = SUM(vmuut1, vmuutk, ftapakk,  MAX(SUM(vevm, -lelvak),0));
	MUU_VAH_VER = SUM(vmuut1, vmuutk, vmuutk, ftapakk,  MAX(SUM(vevm, -lelvak),0));
	...
	DROP csivs fluotap fluotmu ftapakk ftapep ftapepp ftappm ftappmp
		...
		tutmp4 tvahevas tptmuu tvaksp tvuokr tvuokr1 vevm
		vkotita vkotitki vkotitku vkotitp vkotitsv vlahj vmuut1 vmuutk
		vmuutv vthm4 vvvmk1 vvvmk3 vvvmk5 yrtukor hsaiprva haiprva
		hwmky tkoultuk tmtukimk tkopira takuuel tvahep50 tptvs tvahep20 tptsu50
		korosatkg dtyhtep korosapkw
...
/* Vuoden aineistossa äyrit vuosille aineistovuodelle ja kahdelle seuraavalle */
%IF &LVUOSI < 2019 %THEN %DO;
	%KunnVerKerroin(2019, &LVUOSI);
%END;

/* Vuoden 2020 datan kunnallisverotuksen sotemuutoksiin liittyvä korjaus tehdään myöhemmin simuloinnissa */
%IF (&LVUOSI > 2021) and &AVUOSI ne 2020 %THEN %DO;
	%KunnVerKerroin(2021, &LVUOSI);
%END;
...
%MACRO Vero_Simuloi_Data;
    ...
    ****************************************************************************************************************************************************;
	* Tästä eteenpäin ohjelma haarautuu kahteen osaan, joista ensimmäisessä otetaan käyttöön sotemuutokset ja toisessa noudatetaan vanhaa lainsäädäntöä*; 
	****************************************************************************************************************************************************;
	...
	%IF &LVUOSI>=2023 %THEN %DO; 
		...
		******************************************************************;
		...
		/* Verotettava tulo ennen perusvähennystä */
		...
		/* Verotettava tulo perusvähennyksen jälkeen*/
		VERTULO = MAX(SUM(VERTULO1, - pervah), 0); 
		
		**********************************************;
		/* Kunnallisverotuksen osuus */
		**********************************************;

		/* Kunnallis- ja kirkollisveroja */
		/* Muiden kuin Ahvenmaan kunnallisveroprosentteja korjattava */
		IF maakunta ne 21 THEN 
			KUNNVEROA = ((&kunnkerroin * 0.01 * AYRI)- &ProsEro) * VERTULO / 100;
		ELSE 
			KUNNVEROA = &kunnkerroin * 0.01 * AYRI * VERTULO / 100;

		KIRKVEROA = &kirkkerroin * 0.01 * KAYRI * VERTULO / 100;
		...
		ELSE DO;
			...
			IF VERTULO > 0 THEN DO;	
				%SairVakMaksuS(SAIRVAKA, &LVUOSI, &INF, VERTULO, ELAKE, PRAHAMAKSUTULO);
			END;
		ELSE SAIRVAKA = 0;
		END;

		/* Kansaneläkevakuutusmaksu */
		IF MAX(SUM(VERTULO, - tulk), 0) > 0 THEN DO;	
			%KansElVakMaksuS(KEVA, &LVUOSI, &INF, MAX(SUM(VERTULO, - tulk), 0), ELAKE);
		END;
		ELSE KEVA = 0;
		
		**********************************************;
		/* Valtionverotuksen osuus */
		**********************************************;

		/* Valtion tulovero */
		/* Poikkeukset sotemuutoksiin liittyen Ahvenanmaalle ja vuodelle 2022 */
		IF VERTULO > 0 THEN DO;
				IF maakunta=21 THEN DO;
					%ValtTuloVeroS_sote(VALTVEROA, &LVUOSI, &INF, VERTULO,1);
				END;
				ELSE DO;
					IF &LVUOSI=2022 THEN DO;
						%ValtTuloVeroS_sote(VALTVEROA, &LVUOSI, &INF, VERTULO,0);
					END;
					ELSE DO;
						%ValtTuloVeroS(VALTVEROA, &LVUOSI, &INF, VERTULO);
					END;
				END;
		END;
		ELSE VALTVEROA = 0;
		...
		IF SUM(PALKKA1, MUU_TYO, YRITYSTA, -YRVAHA, OSINKOA) > 0 THEN DO;
			%ValtVerTyotVahS_2023(VALTANSVAH, &LVUOSI, &INF, SUM(PALKKA1, MUU_TYO, YRITYSTA, -YRVAHA, OSINKOA), PUHD_ANSIO, ikavu);
		END;
		ELSE VALTANSVAH = 0;
		...
		IF SUM(palksiku_yht, palkomos_yht, tyonosuu_yht) > 0 THEN DO;
			%KotitVahErillJakoS(KOTITVAH, &LVUOSI, &INF, palksiku_remontti, palksiku_koti, palkomos_remontti, palkomos_koti, tyonosuu_remontti, tyonosuu_koti);
		END;
		ELSE KOTITVAH = 0;
		...
		/* Vuodesta 2009 lähtien vähennys vähennetään ensi sijasta valtion 
		...
		IF VEROPUOL = 0 THEN DO;
			VALTVEROD = MAX(VALTVEROC - ELVELV_VAH, 0);
		...
		SET TEMP.&TULOSNIMI_VE (keep = hnro knro VEROPUOL
			ELVELV_VAH1 KOTITVAH1 ALIJHYV1 ALIJHYVERIT1 VALTVEROC1 VALTVEROC1 
			KUNNVEROC1 SAIRVAKC1 KEVC1 KIRKVEROC1 POVEROB1 ELVELV_VAH2 KOTITVAH2 
			ALIJHYV2 ALIJHYVERIT2 VALTVEROC2 VALTVEROC2 KUNNVEROC2 SAIRVAKC2 KEVC2 KIRKVEROC2 POVEROB2); 
		IF VEROPUOL = 1 OR VEROPUOL = 2;
		RUN;

		PROC MEANS DATA = TEMP.VERO_PUOLISOT SUM NOPRINT;
		VAR ELVELV_VAH1 KOTITVAH1 ALIJHYV1 ALIJHYVERIT1 VALTVEROC1 KUNNVEROC1 SAIRVAKC1 KEVC1 KIRKVEROC1 POVEROB1 
		...
		SUM(ELVELV_VAH1 )=ELVELV_VAH1 SUM(KOTITVAH1 )=KOTITVAH1 SUM(ALIJHYV1 )=ALIJHYV1 SUM(ALIJHYVERIT1 )=ALIJHYVERIT1 
		SUM(VALTVEROC1 )=VALTVEROC1 SUM(KUNNVEROC1 )=KUNNVEROC1 SUM(SAIRVAKC1 )=SAIRVAKC1 SUM(KEVC1)=KEVC1 SUM(KIRKVEROC1 )=KIRKVEROC1 
		SUM(POVEROB1)=POVEROB1 SUM(ELVELV_VAH2)=ELVELV_VAH2 SUM(KOTITVAH2)=KOTITVAH2 SUM(ALIJHYV2)=ALIJHYV2 
		...
		*%VahennysSwap(INVVAH_V, VALTVEROC);
		VALTVEROD1 = MAX(SUM(VALTVEROC1, -ELVELV_VAH1), 0);
		VALTVEROD2 = MAX(SUM(VALTVEROC2,  -ELVELV_VAH2), 0);
		...
		ALIJHYVERIT1FINAL ALIJHYVERIT2FINAL
		...
		KEEP hnro OSINKOP1 OSINKOP2 OSINKOP3 OSINKOVAP1 OSINKOVAP2 OSINKOVAP3 OSINKOVAP4
			 ...
			 PUHD_ANSIO PUHD_PO KOTITVAH_DATA ANSIOT_VAH ELTULVAH
			 OPRAHVAH INVVAH_K PALKVAK PRAHAMAKSU PERVAH
			 ...
			 ELVELV_VAH POVEROA POVEROB POVEROC ALIJHYV ALIJHYVERIT 
			 KOTITVAH ANSIOVEROT KAIKKIVEROT KAIKKIVEROT_DATA MAKSP_VEROT YLEVERO 
			 ASKOROT ENSASKOROT MUU_VAH MERIVAH YRVAHA YRVAHP VERTULO VERTULO1;
		...
		ARRAY PISTE
			 ...
			 PRAHAMAKSUTULO VKERROIN ANSIOT POTULOT KOKONTULO THANKKULUT2 PO_VAHENN 
			 PUHD_ANSIO PUHD_PO KOTITVAH_DATA ANSIOT_VAH ELTULVAH
			 OPRAHVAH INVVAH_K PALKVAK PRAHAMAKSU VERTULO PERVAH
			 KUNNVEROA KUNNVEROB KUNNVEROC KUNNVEROD KUNNVEROE KUNNVEROF KUNNVEROG
			 ...
			 VALTANSVAH ELVELV_VAH POVEROA POVEROB POVEROC ALIJHYV ALIJHYVERIT 
			 KOTITVAH ANSIOVEROT KAIKKIVEROT KAIKKIVEROT_DATA MAKSP_VEROT YLEVERO
			 ASKOROT ENSASKOROT MUU_VAH MERIVAH YRVAHA YRVAHP VERTULO VERTULO1;
		DO OVER PISTE;
		...
		ANSIOT_VAH = 'Ansiotulovähennys, MALLI'
		ELTULVAH = 'Eläketulovähennys, MALLI'
		OPRAHVAH = 'Opintorahavähennys, MALLI'
		MERIVAH = 'Merityötulovähennys, MALLI'
		PALKVAK = 'Palkansaajan eläke- ja työttömyysvakuutusmaksut yhteensä, MALLI'
		PRAHAMAKSU = 'Sairausvakuutuksen päivärahamaksu, MALLI'
		PERVAH = 'Perusvähennys, MALLI'
		VERTULO1 = 'Verotettava tulo ennen perusvähennystä (sotemuutokset), MALLI'
		VERTULO = 'Verotettava tulo perusvähennyksen jälkeen (sotemuutokset), MALLI'
...
*****************************************************************************;
* Tästä eteenpäin koodi on yhteistä sotehaaralle ja ei-sotehaaralle         *;
*****************************************************************************;
