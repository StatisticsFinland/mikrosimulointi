...
/* 3. Makro laskee asumislis‰n kuukausitasolla 31.7.2017 asti*/
...
%MACRO AsumLisaKS (tulos, mvuosi, mkuuk, minf, kork, ika, sisaria, asmeno, omatulo, vanhtulo1, vanhtulo2, puoltulo)/
...
IF (kuuid < MDY(7, 1, 1992)) OR (kuuid < MDY(7, 1, 1994) AND &kork = 0) OR kuuid > MDY(7, 1, 2017) THEN temp2 = 0;
...
/* 4. Makro laskee asumislis‰n kuukausitasolla vuosikeskiarvona 31.7.2017 asti*/
...
%MACRO AsumLisaVS (tulos, mvuosi, minf, kork, ika, sisaria, asmeno, omatulo, vanhtulo1, vanhtulo2, puoltulo)/
DES = 'OPINTUKI: Asumislis‰ kuukausitasolla vuosikeskiarvona';

raha = 0;
%DO i = 1 %TO 12;
	%AsumLisaKS(temp, &mvuosi, &i, &minf, &kork, &ika, &sisaria, &asmeno, &omatulo, &vanhtulo1, &vanhtulo2, &puoltulo);
	raha = SUM(raha,  temp);
%END;
&tulos = raha / 12;
DROP raha temp;
