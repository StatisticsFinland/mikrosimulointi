...
%macro ParamTaulut /DES = 'Parametrien listaukset tauluissa';
	...
	%if %length (&POPINTUKI) > 0 %then %do;
		...
		%PaivitaOpintuki(HuoltKor, IndKel, &POPINTUKI, .01, 2019, 100)
		...
		%paivita(ATPerus, IndKel, &POPINTUKI, .01, 2019, 27.54)
		%kytkentaSVTT1(ATVahPros, palkvahpros, &POPINTUKI)
	%end;
	...
	%if %length(&PTTURVA) > 0 %then %do;
		%luo(&PTTURVA);
		%paivita(KorotusOsa, IndKel, &PTTURVA, .01, 2019, 4.74)
	...
	%if %length(&PASUMTUKI) > 0 %then %do;
		%luo(&PASUMTUKI)
		%PaivitaTaannehtiva1(Kattovuokra_1_1, Ind51loka, &PASUMTUKI, 1, 2017, 508)
		%PaivitaTaannehtiva1(Kattovuokra_1_2, Ind51loka, &PASUMTUKI, 1, 2017, 735)
		%PaivitaTaannehtiva1(Kattovuokra_1_3, Ind51loka, &PASUMTUKI, 1, 2017, 937)
		%PaivitaTaannehtiva1(Kattovuokra_1_4, Ind51loka, &PASUMTUKI, 1, 2017, 1095)
		%PaivitaTaannehtiva1(Kattovuokra_1_Plus, Ind51loka, &PASUMTUKI, 1, 2017, 137)
		%PaivitaTaannehtiva1(Kattovuokra_2_1, Ind51loka, &PASUMTUKI, 1, 2017, 492)
		%PaivitaTaannehtiva1(Kattovuokra_2_2, Ind51loka, &PASUMTUKI, 1, 2017, 706)
		%PaivitaTaannehtiva1(Kattovuokra_2_3, Ind51loka, &PASUMTUKI, 1, 2017, 890)
		%PaivitaTaannehtiva1(Kattovuokra_2_4, Ind51loka, &PASUMTUKI, 1, 2017, 1038)
		%PaivitaTaannehtiva1(Kattovuokra_2_Plus, Ind51loka, &PASUMTUKI, 1, 2017, 130)
		%PaivitaTaannehtiva1(Kattovuokra_3_1, Ind51loka, &PASUMTUKI, 1, 2017, 390)
		%PaivitaTaannehtiva1(Kattovuokra_3_2, Ind51loka, &PASUMTUKI, 1, 2017, 570)
		%PaivitaTaannehtiva1(Kattovuokra_3_3, Ind51loka, &PASUMTUKI, 1, 2017, 723)
		%PaivitaTaannehtiva1(Kattovuokra_3_4, Ind51loka, &PASUMTUKI, 1, 2017, 856)
		%PaivitaTaannehtiva1(Kattovuokra_3_Plus, Ind51loka, &PASUMTUKI, 1, 2017, 117)
		...
		%paivita(PerusOmaAikuinenKerroin, IndKel, &PASUMTUKI, 1, 2010, 92)
		%paivita(PerusOmaLapsiKerroin, IndKel, &PASUMTUKI, 1, 2010, 205)
	...
	%if %length(&PKANSEL) > 0 %then %do;
		...
		%paivita(RiLi, IndKel, &PKANSEL, .01, 2001, 1195.92)
	...
	%if %length(&PKOTIHTUKI) > 0 %then %do;
		...
		%paivita(JsRaha1, IndKel, &PKOTIHTUKI, .01, 2014, 243.13)
		%paivita(JsRaha2, IndKel, &PKOTIHTUKI, .01, 2014, 162.09)
...
Ajettuasi tämän PARAMindeksit-ohjelman kokonaisuudessaan, tee parametrien
...
%macro ParamPaivitys (PVUOSI, TVUOSI, INDTAULU,
	...
	%MACRO PaivitaTaannehtiva1(param, ind, ptaulu, rnd, perusvuosi, arvo);
		...
		%DO QD = 1 %TO &MAX;  
			...
			PROC SQL NOPRINT;
				...
				UPDATE &ptaulu
					SET &param =
					%if &EVUOSI = &perusvuosi
					%then %do;
						ROUND
							(
							((&&&ind&EVUOSI)/(&&&ind&E2VUOSI)) * &arvo, &rnd
							)
					%end; 
	...
	%do QD = 1 %to &MAX;
		...
		(2018 eteenpäin) ja vain jos tuloraja nousisi. Poikkeuksena vuosi 2023,
		jonka tarkistus siirtyi vuodelle 2025 vuoden 2024 arvolla.
		---------------------------------------------------------------------*/

				%if %sysfunc(mod(&LVUOSI,2)) = 0 and &LVUOSI >= 2018 and (&LVUOSI ^= 2024) %then %do;
