...
%LET PERHEVAP = 0; 		*Käytetäänkö SAIRVAK-mallissa vuoden 2022 perhevapaauudistuksen lainsäädäntöä (=1 käytetään, =0 ei käytetä) ;
...
%IF &KOKOpoiminta = 1 %THEN %DO;
	...
	(KEEP = hnro knro asko tepalkat toptiot tosinktp anstukor tapur tulkp telps43 tmuukust tepalk
	 ...
	 vvvmk3 vvvmk5 tkoultuk hvamtuk etuki vtukia16 lapsikorotus rielake ryelake kthr kthl ktku
	 ...
	 dtyhtep korosapkw aemk lylen tkapite
	 tansel tkansel tperhel takuuel tkopira tkuntra teleuve
	 toyjmyv toyjmav toyjmyvvap toyjmavvap 
	 tosmetpt tulkelvp tyhtmat tyhtate tyhtmpot tyhtpote tmaat1evyr tmaat1pevyr
	 tliik1evyr tliikpevyr tporo1evyr tyhtmatevyr tyhtateevyr tmaat2evyr
	 tmaat2pevyr tliik2evyr tliik2pevyr tporo2evyr tyhtmpotevyr tyhtpoteevyr ttyosuhdematkal tpolkupyoraetuv 
	);
	...
	MUU_ANSIO = SUM(tepertyok1, tepertyok2, telps41, telps42, telps8, telps1, telps2, telps5, ttyoltuk, tlakko, tpalv2a, tomlis, telvpal, tsuurpu, SUM(MAX(tkoultuk, 0)), tapur);
	...
	SEKAL_VEROTT_TULO = SUM(hkuto, amstipe, hsotav, hasepr, hsotvkor,
		vaklis, korav, elasa, rahsa, apuraha, lassa, kokorve, hlakav, ttyosuhdematkal, tpolkupyoraetuv);
	...
	VEROTT_KANSEL_DATA = SUM(MAX(hvamtuk, 0), etuki, vtukia16, lapsikorotus, rielake, ryelake);
	...
	ELASUMTUKI_DATA = SUM(MAX(aemk, 0));
