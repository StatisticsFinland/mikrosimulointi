...
/* 2.2 Muodostetaan laskennassa tarvittavat yksilötason muuttujat taulukkoon STARTDAT.START_TOIMTUKI */
		...
		DATA STARTDAT.START_TOIMTUKI;
			...
			* Huomioitavan varallisuuden määrä (euroa, vuoden lopussa) ;
			...
			KEEP hnro knro paasoss kuntakoodi jasenia elivtu ikavu
				EIAMSI
...
%IF &TTURVA = 1 OR &TTURVA = 2 OR &KANSEL = 1 OR &OPINTUKI = 1 OR &VERO = 1 OR &KIVERO = 1 OR &LLISA = 1 OR &ELASUMTUKI = 1
	...
	DATA STARTDAT.START_TOIMTUKI;
		...
		%IF &TTURVA = 1 %THEN %DO;
			TEMP.&TULOSNIMI_TT
			(KEEP = hnro YHTTMTUKI PERUSPR ANSIOPR VUORKORV TMTUKILMKOR PERILMAKOR ANSIOILMKOR YLEISTUKI)
		%END;
...
/* 4.1.1 Päätellään, käytetäänkö simuloituja tietoja muista osamalleista vai alkuperäisen datan tietoja */
		...
		* Työttömyysturva korotusosilla ja ilman veronalaisten ei-työtulojen nettomäärän laskentaa varten;
		%IF &TTURVA = 1 %THEN %DO;
			TYOTTURVA_SIMUL = SUM(YHTTMTUKI, PERUSPR, ANSIOPR, VUORKORV, YLEISTUKI);
			TYOTTURVA_ILMKOR_SIMUL = SUM(TMTUKILMKOR, PERILMAKOR, ANSIOILMKOR, VUORKORV, YLEISTUKI);
		%END;
		%ELSE %IF &TTURVA = 2 %THEN %DO;
			TYOTTURVA_SIMUL = SUM(YHTTMTUKI, PERUSPR, ANSIOPR, YLEISTUKI);
			TYOTTURVA_ILMKOR_SIMUL = SUM(TMTUKILMKOR, PERILMAKOR, ANSIOILMKOR, YLEISTUKI);
		%END;
		%ELSE %DO;
...
/* 4.1.2 Tehdään simulointia varten tarvittavat laskutoimitukset */
	...
	* Luodaan aputaulut työtuloista ja tulonhakkimiskuluista leveässä muodossa ;
	...
	proc transpose data = TEMP.TEMP_TOIMTUKI_HENKI out = TEMP.TEMP_IKA_LEVEA prefix = IKA_;
		by knro;
		id JARJ;
		var ikavu;
	run;
	...
	* Yhdistetään kotitaloustauluun ;
	data TEMP.TEMP_TOIMTUKI_KOTI1;
		merge TEMP.TEMP_TOIMTUKI_KOTI1 TEMP.TEMP_TYOTULONETTO_LEVEA TEMP.TEMP_THANKK_LEVEA TEMP.TEMP_IKA_LEVEA;
		by knro;
	run;
...
/* 4.1.3 Lasketaan toimeentulotuki kotitalouksittain */
		...
		MUUTTULOTNETTO_KKS, ASUMISKULUT_KKS, HARKINMENOT_KKS, THANKK_KK_ARRAY, IKA_ARRAY);
