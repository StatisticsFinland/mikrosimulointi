/**********************************************
* Kuvaus: Kansanel‰kkeen simulointimalli 2019 *
...
%IF &START NE 1 %THEN %DO;
	...
	* Tulostaulukoiden esivalinnat ; 
	...
	%LET MUUTTUJAT = takelake TAKUUELA kelkan KANSANELAKE kellaps LAPSIKOROT rili RILISA riyl YLIMRILI kelapu EHOITUKI hlaho LVTUKI 
				     hvamtuk VTUKI rvvm KTUKI LAELAKEDATA LAPSENELAKE LEELAKEDATA LESKENELAKE; * Taulukoitavat muuttujat (summataulukot) ;
...
%MACRO KansEl_Muut_Poiminta;
	...
	%IF &POIMINTA = 1 %THEN %DO;
		...
		DATA STARTDAT.START_KANSEL;
			...
			(
			WHERE = ((ELAKUUK > 0) OR (psmk > 0) OR (kellaps > 0) OR (rili > 0) OR (riyl > 0) OR (kelapu > 0)
			OR (hlaho > 0) OR (hvamtuk > 0)	OR (rvvm > 0) OR (KORJ_LAPEL > 0) OR (KORJ_ALKU > 0)
			OR (jatko > 0))
			...
			VTUKIKUUK KTUKIKUUK KORJ_LAPEL KORJ_ALKU TAKUUEL_TULO ASUSUHT MAMUPTULOT MAMUTULOT LYKKAYSK VARHENP
			);

			ARRAY PISTE 
			rili kelkan takelake kellaps riyl hvamtuk rvvm hlaho kelapu hpelake;
			DO OVER PISTE;
			...
			IF hlepe > 0 OR hleto > 0 OR psmk > 0 THEN LEELAKEDATA = hpelake;
...
DATA TEMP.&TULOSNIMI_KE;
	...
	(
		...
		AND
		(
			/* saa vanhuusel‰kett‰, varhennettua vanhuusel‰kett‰, tyˆkyvyttˆmyysel‰kett‰,
			tyˆttˆmyysel‰kett‰ tai yksilˆllist‰ varhaisel‰kett‰ kansanel‰kkeen‰ */
			(omalaji IN (1,10,2,3,4,5,8,9))
			OR
	...
	*Ylim‰‰r‰iset rintamalis‰t;
	IF riyl > 0 THEN DO;
		...
		ELSE DO;
			...
			END;
			%YlimRintLisaVS(YLIMRILI, &LVUOSI, &INF, LISAOSA, kelkan / ELAKUUK, MUUT_EL / 12);
		END;
	...
	*Vammaistuet;
	...
	END;
	IF rvvm > 0 THEN DO;
		%VammTukiVS(KTUKI, &LVUOSI, &INF, 0, 0, 1, 1);
	...
	*Lapsenel‰ke;
	IF KORJ_LAPEL > 0 THEN DO;
		IF &KDATATULO = 0 THEN DO;	
			%LapsenElakeAVS(LAPSENELAKE, &LVUOSI, &INF, TAYSORPO, LAPSETULO, (perlaji IN (4,5)));
		END;
		ELSE DO;
			%LapsenElakeAVS(LAPSENELAKE, &LVUOSI, &INF, TAYSORPO, SUM(hrelake, htperhe, tuntelpe) / 12, (perlaji IN (4,5)));
		END;
	...
	*Summataan simuloidut leskenel‰kkeet ja lis‰t‰‰n kertasuoritukset (psmk);
	...
	*Maahanmuuttajan erityistuki simuloidaan ulkomailla syntyneille pienituloisille el‰kel‰isille;
	...
	(
		...
		(
			...
			((omalaji IN (1,10)) OR (velaji IN (1,7)))
			OR
			/* saa varhennettua vanhuusel‰kett‰ kansanel‰kkeen‰ */
			(omalaji = 2)
			OR
			...
			KANSANELAKE > 0 
			OR
			/* saa tyˆttˆmyysel‰kett‰ tai yksilˆllist‰ varhaisel‰kett‰ kansanel‰kkeen‰ */
			(omalaji IN (8,9))
			OR 
			/* saa takuuel‰kett‰ */
			(takuuel > 0) 	
...
ARRAY PISTE 
	TAKUUELA KANSANELAKE LAPSIKOROT RILISA YLIMRILI EHOITUKI LVTUKI VTUKI KTUKI 
...
KEEP hnro knro TAKUUELA KANSANELAKE LAPSIKOROT RILISA YLIMRILI EHOITUKI LVTUKI VTUKI KTUKI 
LAELAKEDATA LAPSENELAKE LEELAKEDATA LESKENELAKE LEALKUE LEJATKOE MMTUKI;
...
%IF &START NE 1 %THEN %DO;
	...
	%IF &TULOSLAAJ = 1 %THEN %DO; 
		...
		(KEEP = hnro knro &PAINO kelkan kellaps rili riyl kelapu hlaho hvamtuk rvvm takelake
	...
	* Asetetaan muuttujien 0-arvot tyhjiksi, jotta lukum‰‰r‰t voidaan laskea suoraan ;
	...
	ARRAY PISTE 
		kelkan kellaps rili riyl kelapu hlaho hvamtuk rvvm takelake;
	DO OVER PISTE;
		IF PISTE <= 0 THEN PISTE = .;
	END;
