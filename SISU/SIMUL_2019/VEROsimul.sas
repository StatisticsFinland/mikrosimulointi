...
*  Viimeksi päivitetty: 15.3.2022 MK	  		 				 * 
* ***********************************************************/
...
%IF &POIMINTA = 1 %THEN %DO;
	...
	%IF ((&LVUOSI >= &AVUOSI) AND (&LVUOSI <= %EVAL(&AVUOSI+2))) %THEN %DO; 
		%LET LAYRI = %SUBSTR(&LVUOSI, 3, 2);
		RENAME ayri&LAYRI = AYRI kayri&LAYRI = KAYRI;
	%END;

	%ELSE %IF &LVUOSI < &AVUOSI %THEN %DO;
		%LET LAYRI = %SUBSTR(&AVUOSI, 3, 2);
		RENAME ayri&LAYRI = AYRI kayri&LAYRI = KAYRI; 
	%END;

	%ELSE %IF &LVUOSI > %EVAL(&AVUOSI+2) %THEN %DO;
		%LET LAYRI = %SUBSTR(%EVAL(&AVUOSI+2), 3, 2);
		RENAME ayri&LAYRI = AYRI kayri&LAYRI = KAYRI;
	%END;
...
%IF &LVUOSI < &AVUOSI %THEN %DO;
	%KunnVerKerroin(&AVUOSI, &LVUOSI);
%END;

/* Vuoden 2020 datan kunnallisverotuksen sotemuutoksiin liittyvä korjaus tehdään myöhemmin simuloinnissa */
%IF (&LVUOSI > %EVAL(&AVUOSI+2)) and &AVUOSI ne 2020 %THEN %DO;
	%KunnVerKerroin(%EVAL(&AVUOSI+2), &LVUOSI);
%END;
	...
	****************************************************************************************************************************************************;
	...
	%IF &LVUOSI>=2023 or (&SOTE=1 and &LVUOSI=2022) %THEN %DO; 
		...
		**********************************************;
		...
		IF maakunta ne 21 and &AVUOSI<2021 THEN 
		...
		**********************************************;
		...
		IF SUM(PALKKA1, MUU_TYO, YRITYSTA, -YRVAHA, OSINKOA) > 0 THEN DO;
			%ValtVerTyotVahS_2023(VALTANSVAH, &LVUOSI, &INF, SUM(PALKKA1, MUU_TYO, YRITYSTA, -YRVAHA, OSINKOA), PUHD_ANSIO);
		END;
		ELSE VALTANSVAH = 0;
